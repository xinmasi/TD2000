...
function gohome(){
	if(typeof(mobile_app_home) == "function")
		mobile_app_home();
	else(typeof(mobile_backtomain) == "function")
      mobile_backtomain();
}

function mobile_backtomain(){
   if(typeof UDF_MODULE_IN_APP == "undefined"){
   	location.href = "/pda/main.php?P="+p;   
      return;
   }
   if(UDF_MODULE_IN_APP == "Android"){
      window.Android.backtomain();
      return;
   }else if(UDF_MODULE_IN_APP == "IOS"){
      //alert(UDF_MODULE_IN_APP);

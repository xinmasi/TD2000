...
function AddSecSignFromEkey2()
{
   ...
   {
       if(TANGER_OCX_OBJ.IsNTKOSecSignInstalled())
           TANGER_OCX_OBJ.AddSecSignFromEkey2(TANGER_OCX_user, 0, 0, 1);
   }
}
//开始手写签名
...
function TANGER_OCX_AddDocHeaderExec()
{
    try{
        ...
        if(window.opener)
        {
            ...
            if(parentUrl.indexOf("workflow/list/input_form") >0 || parentUrl.indexOf("approve_center/list/input_form") > 0)
            {
                addFlowData();
            }
        }
...
function TANGER_OCX_SetInfo()
{
    ...
    try
    {
        ...
        {
            TANGER_OCX_OBJ.AddDocTypePlugin(".pdf","PDF.NtkoDocument","4.0.1.0","./ntkopdfdoc.cab",51,true);

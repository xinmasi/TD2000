...
if($IS_MAIN==1)
  ...
  if($ROW=mysql_fetch_array($cursor))
  {
    ...
    if($MANAGER_ID!="")
       $MANAGER_ID_NAME=td_trim(GetUserNameById($MANAGER_ID));    
...
if($TAKER_NAME!="")
   $HTML.= $TAKER_NAME.'<br>'; 
$HTML.='<hr style="margin: 0px;"><p style="word-break: break-all;">';
$HTML.= nl2br($CONTENT);
$HTML.= '</p></div>';
if($FROM==1 &&($MANAGER_ID==$_SESSION["LOGIN_USER_ID"]||($MANAGER_ID==""&&$USER_ID==$_SESSION["LOGIN_USER_ID"]) || $_SESSION["LOGIN_USER_PRIV"]==1))
{
   $FLAG=1;
   $FALG_STATU='<input type="button" value="'._("修改").'" class="btn" onclick=window.open("new_affair.php?AFF_ID='.$AFF_ID.'&FROM='.$FROM.'&IS_MAIN='.$IS_MAIN.'","oa_sub_window","height=480,width=670,status=0,toolbar=no,menubar=no,location=no,left=300,top=100,scrollbars=yes,resizable=yes");>&nbsp;&nbsp;<input type="button" value="'._("删除").'"  class="btn btn-danger" onclick="del_aff_new(\''.$AFF_ID.'\',1,\''.$IS_MAIN.'\')">&nbsp;&nbsp;';
}
else
{
   $FLAG=0;
...
if($AJAX == "1")
{
   ob_end_clean();
   echo $HTML;
   echo '<div align="center" style="float:bottom"><input type="button" value="'._("详情").'" title="'._("生成便签窗口").'" class="btn btn-info" onclick="aff_note(\''.$AFF_ID.'\',\''.$IS_MAIN.'\')">&nbsp;&nbsp;'."$FALG_STATU".'<input type="button" value="'._("关闭").'" class="btn" onclick="HideDialog(\'form_div\');"></div>';
   exit;
}
if($FROM==1)
{
...
<p style="word-break:break-all"><?=nl2br($CONTENT)?></p>
...
?>
<center>
<button type="button" class="btn" onClick="window.open('new_affair.php?AFF_ID=<?=$AFF_ID?>&FROM=<?=$FROM?>','oa_sub_window','height=355,width=500,status=0,toolbar=no,menubar=no,location=no,left=300,top=200,scrollbars=yes,resizable=yes')" ><?=_("修改")?></button>&nbsp;&nbsp;	
<button type="button" onClick="del_aff_new(<?=$AFF_ID?>,1)" class="btn"><?=_("删除")?></button>	
</center>	
<?	
}
?>

...
<?
  if($AFF_ID=="")
  {
    ...
    //同步删除任务中心中日程信息
     ...
     $REFER_URL=$_COOKIE["cal_info_view"]=="" ? "index.php" : $_COOKIE["cal_info_view"].".php";
  ...
  else
  {
    ...
    //............批量删除事务
     ...
     if($ROW=mysql_fetch_array($cursor))
     {
        ...
        if($ADD_TIME!="0000-00-00 00:00:00")
        {
           $querys="select AFF_ID from AFFAIR where ADD_TIME='$ADD_TIME' and MANAGER_ID='$MANAGER_ID'";
           $cursors=exequery(TD::conn(),$querys,$QUERY_MASTER);
           $AFF_ID_STR="";
           while($ROWS=mysql_fetch_array($cursors))
           {
          ...
          if(substr($AFF_ID_STR,-1)==",")
             $AFF_ID_STR=substr($AFF_ID_STR,0,-1);
    ...
    //.......................
     $query="delete from AFFAIR where AFF_ID in ($AFF_ID_STR)";
     exequery(TD::conn(),$query);
     $REFER_URL=$_COOKIE["cal_info_view"]=="" ? "index.php" : $_COOKIE["cal_info_view"].".php";
     header("location: $REFER_URL?OVER_STATUS=$OVER_STATUS&YEAR=$YEAR&MONTH=$MONTH&DAY=$DAY&DEPT_ID=$DEPT_ID&IS_MAIN=1");
  }

...
<style>
.textareaBig{
	height:63px; line-height:21px; padding:4px 4px;outline:none; width:245px;
}
.tasktitle{
...
(function($){
	jQuery(document).ready(function(){
		...
		if($("#SMS_REMIND").attr("checked")!="checked")
		{
			$("#remind_time").hide();
		}else
		{
...
?>
   <span class="big3" style="padding-left:30px;" title="<?=$s_show_name_str?>"> <? if($TASK_ID=="")echo _("新建");else echo _("编辑");?><?=_("任务")?>-<? if($TASK_ID!="")
   echo trim($USER_NAME_STR,",");else echo $USER_NAME;?></span>
</div>

 <div class="head_top">
  <form action="<?if($TASK_ID=="")echo "task_insert";else echo"task_update";?>.php"  method="post" name="form1" class="form-horizontal" onSubmit="return CheckForm();">
    <div class="control-group">
      <label class="control-label" for="TASK_NO"><?=_("排序号：")?></label>
      <div class="controls">
        <input type="text" name="TASK_NO" id="TASK_NO" class="input-small" value="<?=$TASK_NO?>"><font><?=_("(排序号应为数字型)")?></font>
      </div>
    </div>
    <div class="control-group">
      <label class="control-label" for="SUBJECT"><?=_("任务标题：")?></label>
      <div class="controls">
        <input type="text" name="SUBJECT" id="SUBJECT" value="<?=$SUBJECT?>" style="width:243px;outline:none"> <font color=red><?=_("(*)")?></font>
      </div>
    </div>
    ...
    <div class="control-group" style="clear:both">
      ...
      <div class="controls">
        <input class="input input-small calendar-startdate valtype" name="BEGIN_DATE" value="<?=$BEGIN_DATE?>" placeholder="开始日期" type="text" data-valtype="placeholder" style="outline:none">
        <?=_("至")?>
        <input class="input input-small calendar-startdate valtype" name="END_DATE" value="<?=$END_DATE?>" placeholder="结束日期" style="margin-left: 22px;outline:none" type="text" data-valtype="placeholder">
    ...
    <div class="control-group">
      ...
      <div class="controls">
        <textarea name="CONTENT" class="textareaBig"><?=$CONTENT?></textarea>
      </div>
    </div>
    ...
    <div class="control-group" style="float:left;">
        ...
        <div class="controls">
        <input type="text" class="input-small" name="RATE"  id="RATE" value="<?=$RATE?>" style="outline:none"><span class="help-inline">%</span>
        </div>
    </div>
    <div class="control-group" style="clear:both">
        <label class="control-label"><?=_("完成时间：")?></label>
        <div class="controls">
        <input class="input input-small calendar-startdate valtype" name="FINISH_TIME" placeholder="完成日期" value="<?=$FINISH_TIME_NEW?>" type="text" data-valtype="placeholder" style="outline:none">
    ...
    <div class="control-group" style="width:325px; float:left;">
        ...
        <div class="controls">
          <input type="text" class="input-small" name="TOTAL_TIME" id="TOTAL_TIME" value="<?=$TOTAL_TIME?>" style="outline:none">
    ...
    <div class="control-group" style="width:325px;float:left;">
        ...
        <div class="controls" style="margin-left:0px;">
          <input type="text" class="input-small" style="margin-left:20px;outline:none" name="USE_TIME" id="USE_TIME" value="<?=$USE_TIME?>">
          <span class="help-inline"><?=_("小时")?></span>
        </div> 
    </div>
...
<?=sms_remind(5);?>
    ...
    <div class="controls buttonGroupBottom">
        ...
        <button type="button" onClick="parent.close();" class="btn"><?=_("关闭")?></button>
    </div>

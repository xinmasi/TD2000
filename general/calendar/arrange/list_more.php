...
?>
<table class="table table-bordered table-hover" width="90%" style=" background-color:#fff">
  <thead align="center" style="background-color:rgb(231, 231, 231)">
    <th width="40"><?=_("选择")?></th>
    <th width="80"><?=_("时间")?></th>
    <th><?=_("内容")?></th>
    <th><?=_("类型")?></th>
    <td width="120"><?=_("操作")?></th>
  </thead>
<?
 //============================ 显示日程安排 =======================================
 $CAL_COUNT=0;
 $DATE_NOW=date("Y-m-d",strtotime("-1 day")); 
 $query = "SELECT * from CALENDAR where USER_ID='".$_SESSION["LOGIN_USER_ID"]."'".$CONDITION_STR."  and  from_unixtime(END_TIME,'%Y-%m-%d')<='$DATE_NOW' order by CAL_ID desc  limit $PAGE_START,$PAGE_SIZE ";
 $cursor= exequery(TD::conn(),$query);
...
?>
  <tr id="list_tr_<?=$CAL_ID?>" align="center" class="" title="<?=$CAL_TITLE?>">
    ...
    <td align="left" width="50%"><span class="CalLevel<?=$CAL_LEVEL?>" title="<?=cal_level_desc($CAL_LEVEL)?>">&nbsp</span><a id="cal_<?=$CAL_ID?>" href="javascript:my_note(<?=$CAL_ID?>);" status="<?=$OVER_STATUS?>" style="color:<?=$STATUS_COLOR?>;"><?=$CONTENT?></a></td>
    <td><?=$CAL_TYPE_DESC?></td>
    <td>
<?
...
?>
  <tr class="TableControl">
    <td colspan="6"><label class="checkbox" for="allbox_for" style="float:left;"><input type="checkbox" name="allbox" id="allbox_for" onClick="check_all();"><?=_("全选")?></label>&nbsp;

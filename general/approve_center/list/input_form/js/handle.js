...
function showInstrustDiv() {
    ...
    var run_name_new1 = run_name_new;
    ...
    jQuery('#myModal_intrust').find('.modal-body').load('/module/approve_center/engine/data/work_run_intrust.php?run_id='+run_id+'&flow_id='+flow_id+'&flow_prcs='+flow_prcs+'&prcs_id='+prcs_id+'&prcs_key_id='+prcs_key_id, function() {
        ...
        if (Symbol) 
        {
            ...
            var run_name = jQuery('#run_name_old').val() || run_name_new1;
        }
...
jQuery(document).ready(function() {
    ...
    jQuery('#fallback').click(function() {
		...
		//异步加载在IE下可能存在无法取消的问题，改为同步
		jQuery.ajax({
            type: 'GET',
            url: 'data/getflowprcsdata.php?flow_id='+flow_id+'&allow_back='+allow_back+'&run_id='+run_id+'&flow_prcs='+flow_prcs+'&prcs_id='+prcs_id+'&prcs_key_id='+prcs_key_id,
            cache: true,
            async: false,
    ...
    jQuery("#add_sign_user_ok").click(function() { // 增加会签人 添加
        ...
        var sign_user_cause = jQuery.trim(jQuery('#SIGN_CAUSE').val());
        if (sign_user_ids == '') {
            alert(td_lang.system.workflow.msg_31);
            return false;
        }
        // var data = "sign_user_ids=" + sign_user_ids + "&sign_user_names=" + sign_user_names + "&flow_id=" + flow_id + "&run_id=" + run_id + "&prcs_id=" + prcs_id + "&flow_prcs=" + flow_prcs + "&op_flag=" + op_flag + "&flow_type=" + flow_type+"&sign_user_cause="+sign_user_cause;
        jQuery.ajax({
            url: "data/addsign.php",
            type: "GET",
            data: {
                 'sign_user_ids' :sign_user_ids,
                 'sign_user_names' :sign_user_names,
                 'flow_id' :flow_id,
                 'run_id'  :run_id,
                 'flow_prcs' :flow_prcs,
                 'prcs_id' :prcs_id,
                 'op_flag' :op_flag,
                 'flow_type' :flow_type,
                 'sign_user_cause' :sign_user_cause
            },
            success: function(data) {
				if( data == "nosignpriv"){
                    ...
                    if (data.add_user) {
                        ...
                        {   
							...
							if(op_flag == 1){
                                ...
                                {
                                    //sign_user_cause = sign_user_cause.replace(' /\s+/',sign_user_cause);
                                    var temp_str = '<span id=\''+add_user_id_arr[count]+'\'>' + add_user_arr[count] + '<img src="/static/images/edit.gif" title=\"增加理由：'+sign_user_cause+'\" style="cursor:pointer;" align=absmiddle >'+'<img src="/static/images/delete.gif" onclick="cancel_do(\''+run_id+'\',\''+prcs_id+'\',\''+flow_prcs+'\',\''+add_user_id_arr[count]+'\')"></span>';
                                }
                                jQuery('#SIGN_CAUSE').val("");
        ...
        else 
        {
            ...
            try {
                jQuery("li[id^='next_prcs_'][class*='active']").each(function(i, v) {
                    ...
                    if (obj_choose_prcs.attr("id") === "next_prcs_0" && (obj_choose_prcs.attr('prcs_back') == 0 || typeof(obj_choose_prcs.attr('prcs_back')) == 'undefined')) {
                        chose_obj.val("");
                    } else {
...
(function($) {
    ...
    FreePrcsSelector.prototype = {
        ...
        renderCtrl: function() {
            ...
            this.$add = $('<button class="btn btn-small btn-info" style="display:inline-block">增加自由步骤</button>').appendTo($btns);
            this.$clear = $('<button class="btn btn-small btn-danger pull-right">重置自由步骤</button>').appendTo($btns).css('margin-left', 10);
...
function addPrcsJsonDatas(data) {
    ...
    var templateNext = jQuery.templates("#workNextPrcsTmpl");
    var htmlOutputNext = templateNext.render(Data);
    jQuery("#workPrcsData").html(htmlOutputNext);
    ...
    jQuery(".workflow-node").click(function() {
        ...
        if (sync_deal == 0) {
            ...
            if ($this.attr('user_lock') !== '0') {
                ...
                if(($this.attr('auto_type')!=="" || $this.attr('prcs_type') == 4)&&jQuery(".user-tags").length!==0)
                {
            ...
            if (prcs_info_arr[2] === "0" && typeof $this.attr("prcs_back") == 'undefined') { //结束
                ...
                if ($this.hasClass('active')) { //取消
                    ...
                    $li_obj.each(function(i, v) {
                        ...
                        {
                            if(($v.attr('auto_type') !== "" || $this.attr('prcs_type') == 4) && jQuery(".user-tags").length !== 0)
                            {
                                jQuery('#chose_user' + prcs_num).click(function() {//yzx不允许修改默认人员
                                    alert(td_lang.general.workflow.msg_247);
                                    return false;
        ...
        if(numHidden > 0)
        {
            ...
            jQuery('#con_hidden').click(function()
            {
                ...
                if(hiddenFlag == 0)
                {
                    ...
                    {
                        // if(data_hidden_arr[i].hasOwnProperty('prcs_in_condition') && data_hidden_arr[i].prcs_in_condition != "" )
                        // {

                            // jQuery('#next_prcs_'+data_hidden_arr[i].prcs_num).css('display','none');
                ...
                }else if(hiddenFlag == 1)
                {
                    ...
                    for (var i = 0; i < hiddenArr.length; i++)
                    {
                        ...
                        // {
                            ...
                            //jQuery('#next_prcs_'+data_hidden_arr[i].prcs_num).css('display','block');
                            // jQuery('#next_prcs_'+data_hidden_arr[i].prcs_num).animate({height: 'show'});

    ...
    if(!MENU_FLOW_ID){
        jQuery.getJSON("data/getcolumns.php", {
            ...
            function(columns){
                ...
                var LEAVE_USER_ID = jQuery('#LEAVE_EXPORT_TYPE').find('option:selected').val();//离职人员
                ...
                var url = 'data/getdata.php?action=list_allwork&flow_id='+FLOW_ID+'&begin_time='+BEGIN_TIME+'&end_time='+END_TIME+'&run_id='+RUN_ID+'&flow_status='+FLOW_STATUS+'&run_name='+RUN_NAME+'&flow_query_type='+FLOW_QUERY_TYPE+'&to_id='+TO_ID+'&work_level='+WORK_LEVEL+'&leave_user_id='+LEAVE_USER_ID;
...
function set_user()
{
    ...
    var LEAVE_USER = document.getElementById("LEAVE_QUERY_TYPE");
    var LEAVE_EXPORT_USER = document.getElementById("LEAVE_EXPORT_TYPE");
    var query_export_area= jQuery("#exp_flow_query_type").find("option:selected").val();
    ...
    if(document.form1.FLOW_QUERY_TYPE.value==10)
    {
        LEAVE_USER.style.display="inline";
    }else
    {
        LEAVE_USER.style.display="none";
    }
    if(query_export_area == 10)
    {
        LEAVE_EXPORT_USER.style.display="inline";
    }else
    {
        LEAVE_EXPORT_USER.style.display="none";
    }

...
{
	var module_type = jQuery('#archive_data').find('option:selected').attr('module_id');
	var newFlowId = module_type == 'workflow' ? jQuery('#FLOW_ID').find('option:selected').attr('old_id') : jQuery.trim(jQuery('#FLOW_ID').val());
    var searchStr ="FLOW_ID="+newFlowId;
...
{
	var module_type = jQuery('#archive_data').find('option:selected').attr('module_id');
	var old_id = jQuery('#exp_flow_id').find('option:selected').attr('old_id');
	var url = '';
	if(module_type == 'workflow')
	{
		url = "/general/workflow/query/data/archive_doexport.php";
		jQuery("#exp_flow_id_old").val(old_id);
	}else
	{
		url = "/general/approve_center/query/data/archive_doexport.php";
	}
...
function check_form()
{
    ...
    jQuery.getJSON("data/archive_getcolumns.php?"+searchStr, {
        ...
        function(columns){
			...
			if(module_type == 'workflow')
			{
				url = "/general/workflow/query/data/archive_getdata.php?"+searchStr+"&ARCHIVE_ID="+archive_id+"&MODULE_TYPE="+module_type;
			}else
			{
				url = "data/archive_getdata.php?"+searchStr+"&ARCHIVE_ID="+archive_id+"&MODULE_TYPE="+module_type;
			}

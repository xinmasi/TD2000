jQuery(document).ready(function(){
    ...
    jQuery.getJSON("/general/approve_center/get_flow_list.inc.php",{"FLOW_ID":MENU_FLOW_ID,"action":4,"root":true},function(jsonData){
        ...
        jQuery.each(jsonData,function(i,t){
            jQuery("#FLOW_ID").append('<option value="'+t.value+'" category="'+t.category+'" node="'+t.node+'" '+t.selected+' level="'+t.level+'" old_id="'+t.old_id+'">'+t.txt+'</option>');
        	jQuery("#exp_flow_id").append('<option value="'+t.value+'" category="'+t.category+'" node="'+t.node+'" '+t.selected+' level="'+t.level+'" old_id="'+t.old_id+'">'+t.txt+'</option>');
		...
		jQuery.getJSON("archive_time.php","",function(jsonData){
			...
			jQuery.each(jsonData,function(i,t){
				jQuery("#archive_data").append('<option value="'+t.value+'" module_id="'+t.module+'">'+t.txt+'</option>');
			});
			check_form();
		});	  
		...
		jQuery.getJSON("archive_time.php","",function(jsonData){
			...
			jQuery.each(jsonData,function(i,t){
				jQuery("#exp_archive_data").append('<option value="'+t.value+'" module_id="'+t.module+'">'+t.txt+'</option>');
			});
...
function export_run_sel()
{
	...
	var module_type = jQuery('#archive_data').find('option:selected').attr('module_id') == 'workflow' ? 'workflow' : 'approve_center';
	var url = module_type == 'approve_center' ? "archive_excel_data.php" : "/general/workflow/query/archive_excel_data.php";
	...
	var flowId;
	var oldFlowId = jQuery("#FLOW_ID").find('option:selected').attr('old_id');
    oldFlowId == -1 ? flowId = jQuery("#FLOW_ID").val() : flowId = oldFlowId;
    window.location = url+"?FLOW_ID="+flowId+"&RUN_ID_STR="+runId+"&archive_data="+jQuery("#archive_data").val();
...
function zip_run_sel()
{
	var runId=jQuery("#gridTable").jqGrid('getGridParam','selarrrow');
	var module_type = jQuery('#archive_data').find('option:selected').attr('module_id') == 'workflow' ? 'workflow' : 'approve_center';
	var url = module_type == 'approve_center' ? "archive_export_zip.php" : "/general/workflow/query/archive_export_zip.php";
	if(runId == "" || runId == "undefined")
	{
		alert(td_lang.general.workflow.msg_56);
		return false
	}
    window.location = url+"?RUN_ID_STR="+runId+"&archive_data="+jQuery("#archive_data").val();

...
function delete_uvehicle(VU_ID,back_str)
{
  ...
  {
     URL="delete.php?VU_ID=" + VU_ID + back_str;
     window.location=URL;
  }
}
...
function delete_item(back_str)
{
	...
	if(window.confirm(msg))
	{
		url="delete.php?VU_ID="+ delete_str + '&delete_all=1' + back_str;
    	location=url;
  	}
}
...
while($ROW=mysql_fetch_array($cursor))
{
   ...
   $query2 = "select OPERATOR_NAME from VEHICLE_OPERATOR where OPERATOR_ID='1'";
   $cursor2=exequery(TD::conn(),$query2);
   if($ROW2=mysql_fetch_array($cursor2))
      $MANAGERS=$ROW2["OPERATOR_NAME"];

   if(find_id($MANAGERS,$_SESSION["LOGIN_USER_ID"]))
      $query2 = "SELECT V_NUM from VEHICLE where V_ID='$V_ID'";
   else
      $query2 = "SELECT V_NUM from VEHICLE where V_ID='$V_ID' and (DEPT_RANGE = 'ALL_DEPT' or find_in_set('".$_SESSION["LOGIN_DEPT_ID"]."',DEPT_RANGE) or find_in_set('".$_SESSION["LOGIN_USER_ID"]."',USER_RANGE) or DEPT_RANGE='' and USER_RANGE='')";
   $cursor2= exequery(TD::conn(),$query2);
   if($ROW2=mysql_fetch_array($cursor2))
      $V_NUM=$ROW2["V_NUM"];
   else
...
if($ROW1=mysql_fetch_array($cursor1))
{
   ...
   {
?>
        <a href="javascript:;" onClick="window.open('modify.php?VU_ID=<?=$VU_ID?>','','height=380,width=680,status=1,toolbar=no,menubar=no,location=no,scrollbars=yes,top=200,left=280,resizable=yes');"><?=_("修改")?></a>
<?
   }
}

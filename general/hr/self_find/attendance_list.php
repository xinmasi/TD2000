...
$query1="SELECT USER_NAME,DEPT_ID,UID from USER where USER_ID='$USER_ID'";
...
if($duty_type!='99')
{
    ...
    {
        if(find_id($GENERAL,$WEEK) && $duty_type=="")
    ...
    <table class="table table-bordered"  width="95%" align="center">
        ...
        for($I=1;$I<=6;$I++)
        {
            ...
            if($HOLIDAY=="")
            {
                $query="SELECT * FROM attend_leave WHERE USER_ID='$USER_ID' AND (ALLOW='1' or ALLOW='3') AND LEAVE_DATE1<='$SOME_DATE $DUTY_TIME_I' AND LEAVE_DATE2>='$SOME_DATE $DUTY_TIME_I'";
                $cursor= exequery(TD::conn(),$query);
                if($ROW=mysql_fetch_array($cursor))
            ...
            <tr class="">
                ...
                <td nowrap align="center">
                    ...
                    if($REGISTER_TIME=="")
                    {
                        ...
                        elseif(strtotime($DUTY_TIME_I)<strtotime(date('H:i:s',time())))
                        {
                            echo "未登记";
                        }
                    }
    ...
    ?>
    <form action="attendance_search.php" name="form1" onSubmit="return CheckForm1();">
        <table align="center" class="table table-bordered" width="95%">
            ...
            <tr>
                <td>
                    <?=_("起始日期：")?><input type="text" name="DATE1" size="10" maxlength="10" class="" id="start_time" value="<?=$CUR_DATE_FIRST?>" onClick="WdatePicker({maxDate:'%y-%M-%d %H-%m-%s'})"/>
                    &nbsp;
                    <?=_("截止日期：")?><input type="text" name="DATE2" size="10" maxlength="10" class="" value="<?=$CUR_DATE?>" onClick="WdatePicker({minDate:'#F{$dp.$D(\'start_time\')}'})"/>
                </td>
                <td>
                    <input type="hidden" name="USER_ID" value="<?=$USER_ID?>">
                    <input type="submit" value=<?=_("统计")?> class="btn btn-primary">
                </td>
            </tr>
        </table>
    </form>

    <form action="attendance_some_day.php" name="form2" onSubmit="return CheckForm2();">
        <table align="center" class="table table-bordered" width=450>
            <tr  >
                <td colspan=2>
                    <?=_("上下班登记查询")?><?=$NO_DUTY_STR?>
                </td>
            </tr>
            <tr>
                <td class=TableData>

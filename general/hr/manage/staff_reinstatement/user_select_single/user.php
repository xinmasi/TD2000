...
<?
$query="select DEPT_ID from HR_STAFF_INFO where USER_ID='$LEAVE_PERSON'";
...
if($ROW=mysql_fetch_array($cursor))
 ...
 if($USER_PRIV=="")
 {
    ...
    $query = "SELECT UID,HR_STAFF_INFO.USER_ID,USER_NAME,PRIV_NO from USER_PRIV,USER left join HR_STAFF_INFO on USER.USER_ID=HR_STAFF_INFO.USER_ID where USER.DEPT_ID='0' and USER.USER_PRIV=USER_PRIV.USER_PRIV";
    if(!$MANAGE_FLAG)
...
<tr class="TableData" style="cursor:pointer">
  <td class="menulines" align="center" onClick="javascript:add_user('<?=$USER_ID?>','<?=$USER_NAME?>')"><?=$USER_NAME?><span id="attend_status_<?=$UID?>" title="<?=$USER_ID?>" style="color:#FF0000;"><?if(array_key_exists($UID,$ONLINE_USER_ARRAY)) echo _("(在线)");?></span></td>
</tr>
<?
 }

...
?>
<tr class="TableData">
  <td align="center" onclick="javascript:parent.user.location='user.php?MODULE_ID=<?=$MODULE_ID?>&TO_ID=<?=$TO_ID?>&TO_NAME=<?=$TO_NAME?>&FORM_NAME=<?=$FORM_NAME?>&USER_PRIV=<?=$USER_PRIV?>&POST_PRIV=<?=$POST_PRIV?>&POST_DEPT=<?=$POST_DEPT?>&MANAGE_FLAG=<?=$MANAGE_FLAG?>&LEAVE_PERSON=<?=$LEAVE_PERSON?>';" style="cursor:pointer"><?=$PRIV_NAME?></td>
</tr>
<?
 }

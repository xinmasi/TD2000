...
function add_user(user_id,user_name,dept_id,dept_name,staff_card_no)
{
  TO_VAL=to_id.value;
  if(TO_VAL.indexOf(","+user_id+",")<0 && TO_VAL.indexOf(user_id+",")!=0)
  {
    ...
    parent_window.form1.LICENSE_DEPT.value=dept_id;
    parent_window.form1.LICENSE_DEPT_NAME.value=dept_name;
    parent_window.form1.STAFF_CARD_NO.value=staff_card_no;
...
if($MODULE_ID=="2")
 ...
 {
    $query = "SELECT UID,USER_ID,DEPT_ID,USER_NAME,DEPT_ID from USER where USER_PRIV='$USER_PRIV' and DEPT_ID!=0 and USER.USER_PRIV=USER_PRIV.USER_PRIV";
    if(!$MANAGE_FLAG)
       $query .= " and NOT_LOGIN='0'";
    if($DEPT_PRIV=="3"||$DEPT_PRIV=="4")
      $query .= " and find_in_set(USER.USER_ID,'$USER_ID_STR')";
...
// file_put_contents("a.txt",$query);
 ...
 while($ROW=mysql_fetch_array($cursor))
 {
	...
	$query2="SELECT STAFF_CARD_NO FROM HR_STAFF_INFO WHERE USER_ID='$USER_ID'";
	$cursor2= exequery(TD::conn(),$query2);
	if($ROW2=mysql_fetch_array($cursor2)){
		$STAFF_CARD_NO=$ROW2['STAFF_CARD_NO'];
	}
...
<tr class="TableData" style="cursor:pointer">
  <td class="menulines" align="center" onclick="javascript:add_user('<?=$USER_ID?>','<?=$USER_NAME?>','<?=$DEPT_ID?>','<?=$DEPT_NAME?>','<?=$STAFF_CARD_NO?>')"><?=$USER_NAME?><span id="attend_status_<?=$UID?>" title="<?=$USER_ID?>" style="color:#FF0000;"><?if(array_key_exists($UID,TD::get_cache('SYS_ONLINE_USER'))) echo _("(在线)");?></span></td>
</tr>
<?
 }

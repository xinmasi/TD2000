...
{
   $query = "SELECT count(*) from HR_STAFF_LABOR_SKILLS where STAFF_NAME='$USER_ID' ";
...
?>
  ...
  <div align="center">
     <input type="button" value="<?=_("新建劳动技能信息")?>" class="BigButton" onClick="window.open('skills_new.php?USER_ID=<?=$USER_ID?>','','height=500,width=820,status=1,toolbar=no,menubar=no,location=no,scrollbars=yes,left=200,top=100,resizable=yes');">
  </div>
  <br>
<?
if($TOTAL_ITEMS>0)
...
$query = "SELECT * from HR_STAFF_LABOR_SKILLS where STAFF_NAME='$USER_ID' order by ADD_TIME desc limit $PAGE_START, $PAGE_SIZE";
$cursor= exequery(TD::conn(),$query);
$SKILLS_COUNT=0;
...
?>
   <tr class="TableData">
      ...
      <a href="javascript:;" onClick="window.open('skills_modify.php?SKILLS_ID=<?=$SKILLS_ID?>','','height=500,width=820,status=1,toolbar=no,menubar=no,location=no,scrollbars=yes,left=200,top=100,resizable=yes');"><?=_("编辑")?></a>&nbsp;
      </td>
   </tr>
<?
}

...
{
   $query = "SELECT count(*) from HR_STAFF_TRANSFER where TRANSFER_PERSON='$USER_ID' ";
...
?>
  ...
  <div align="center">
     <input type="button" value="<?=_("新建人事调动信息")?>" class="BigButton" onClick="window.open('transfer_new.php?USER_ID=<?=$USER_ID?>','','height=500,width=820,status=1,toolbar=no,menubar=no,location=no,scrollbars=yes,left=200,top=100,resizable=yes');">
  </div>
  <br>
<?
if($TOTAL_ITEMS>0)
...
$query = "SELECT * from HR_STAFF_TRANSFER where TRANSFER_PERSON='$USER_ID' order by ADD_TIME desc limit $PAGE_START, $PAGE_SIZE";
$cursor= exequery(TD::conn(),$query);
$TRANSFER_COUNT=0;
...
?>
   <tr class="TableData">
      ...
      <td nowrap align="center"><?=$TRANSFER_DATE?></td>
      <td nowrap align="center"><?=$TRANSFER_EFFECTIVE_DATE?></td>
      ...
      <a href="javascript:;" onClick="window.open('transfer_modify.php?TRANSFER_ID=<?=$TRANSFER_ID?>','','height=500,width=820,status=1,toolbar=no,menubar=no,location=no,scrollbars=yes,left=200,top=100,resizable=yes');"><?=_("编辑")?></a>&nbsp;
      </td>
   </tr>
<?
}

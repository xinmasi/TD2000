...
<tr class="TableData" style="cursor:pointer">
  <td class="menulines" align="center" onclick="javascript:add_user('<?=$USER_ID?>','<?=$USER_NAME?>','<?=$DEPT_ID?>','<?=$DEPT_NAME?>')"><?=$USER_NAME?><span id="attend_status_<?=$UID?>" title="<?=$USER_ID?>" style="color:#FF0000;"><?if(array_key_exists($UID,TD::get_cache('SYS_ONLINE_USER'))) echo _("(在线)");?></span></td>
</tr>
<?
 }

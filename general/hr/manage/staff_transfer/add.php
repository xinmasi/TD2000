...
$C = preg_match('/<img.*?\ssrc=\\\"\/inc\/attach.php\?(.*)MODULE=upload_temp/i',$TRAN_REASON);
$TRAN_REASON = replace_attach_url($TRAN_REASON);
if($C==1)
{
...
$DEPT_HR_MANAGER1 = $TRANSFER_PERSON.',';
...
foreach($PERSON_ARRAY as $PERSON){
    ...
    $query = "update  HR_STAFF_INFO SET DEPT_ID='$TRAN_DEPT_AFTER',JOB_POSITION='$TRAN_POSITION_AFTER' where USER_ID = '$PERSON'";
    exequery(TD::conn(),$query);
    //------- 更新员工角色 --------
    if($role != "")
    {

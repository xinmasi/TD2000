...
{
    AddImage2Editor('TRAN_REASON', src);
}
...
if($ROW=mysql_fetch_array($cursor))
{
   ...
   {
      $SELECT_FLAG=1;
      $query = "SELECT STAFF_NAME from HR_STAFF_INFO where USER_ID='$TRANSFER_PERSON'";
      $cursor= exequery(TD::conn(),$query);
      if($ROW=mysql_fetch_array($cursor))
         $TRANSFER_PERSON_NAME=$ROW["STAFF_NAME"];
   }
...
?>
    ...
    <tr id="CONTRACT_END_DATE">
                ...
                <?
                  $query = "SELECT USER_PRIV from user where USER_ID='".$TRANSFER_PERSON."' ;";
                  $cursor= exequery(TD::conn(),$query);
                  if($ROW=mysql_fetch_array($cursor))
                  {

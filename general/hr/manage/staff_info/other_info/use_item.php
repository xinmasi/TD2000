...
<?
 $query = "SELECT * from OFFICE_TRANSHISTORY,OFFICE_PRODUCTS where BORROWER='$USER_ID' and  OFFICE_TRANSHISTORY.PRO_ID=OFFICE_PRODUCTS.PRO_ID AND(TRANS_FLAG=1 OR TRANS_FLAG=2 OR TRANS_FLAG=3) order by TRANS_DATE";
...
<?
  ...
  else
  {
    Message("",_("无领用物品记录"));
?>
    <div align="center"><input type="button" value="<?=_("关闭")?>" class="BigButton" onclick="window.close();"/></div>
<?
  }
?>
</body>

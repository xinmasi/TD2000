...
{
   $query = "SELECT count(*) from HR_STAFF_LEAVE where LEAVE_PERSON='$USER_ID' ";
...
?>
  ...
  <div align="center">
     <input type="button" value="<?=_("新建员工离职信息")?>" class="BigButton" onClick="window.open('leave_new.php?USER_ID=<?=$USER_ID?>','','height=500,width=820,status=1,toolbar=no,menubar=no,location=no,scrollbars=yes,left=200,top=100,resizable=yes');">
  </div>
  <br>
<?
if($TOTAL_ITEMS>0)
...
$query = "SELECT * from HR_STAFF_LEAVE where LEAVE_PERSON='$USER_ID' order by ADD_TIME desc limit $PAGE_START, $PAGE_SIZE";
$cursor= exequery(TD::conn(),$query);
$LEAVE_COUNT=0;
...
?>
   <tr class="TableData">
      ...
      <a href="javascript:;" onClick="window.open('leave_modify.php?LEAVE_ID=<?=$LEAVE_ID?>','','height=500,width=820,status=1,toolbar=no,menubar=no,location=no,scrollbars=yes,left=200,top=100,resizable=yes');"><?=_("编辑")?></a>&nbsp;
      </td>
   </tr>
<?
}

...
{
    AddImage2Editor('QUIT_REASON', src);
}
...
if($ROW=mysql_fetch_array($cursor))
{
  ...
  if($LEAVE_PERSON_NAME=="")
  {
      ...
      if($ROW1=mysql_fetch_array($cursor1))
         $LEAVE_PERSON_NAME=$ROW1["STAFF_NAME"];
   }
...
<form action="update.php"  method="post" name="form1" id="form1"  enctype="multipart/form-data" onSubmit="return CheckForm();">
 <table class="TableBlock" width="80%" align="center">
   <tr>
      ...
      <td class="TableData">
        <input type="text" name="LEAVE_PERSON_NAME" size="15" class="BigStatic validate[required]" data-prompt-position="centerRight:0,-8" readonly value="<?=$LEAVE_PERSON_NAME?>">&nbsp;
...
?>
    ...
    <tr>
      ...
      <td class="TableData">
       <input type="text" name="APPLICATION_DATE" id="apple_date" size="15" maxlength="10" class="BigInput" value="<?=$APPLICATION_DATE?>" onClick="WdatePicker()"/>
      </td>
    </tr>
    <tr>
      <td nowrap class="TableData"> <?=_("拟离职日期：")?></td>
      <td class="TableData">
        <input type="text" name="QUIT_TIME_PLAN" size="15" maxlength="10" class="BigInput validate[required]" data-prompt-position="centerRight:0,-8"  value="<?=$QUIT_TIME_PLAN?>" onClick="WdatePicker({minDate:'#F{$dp.$D(\'apple_date\')}'})"/>
      </td>
     ...
     <tr>
        ...
        <td class="TableData" ><input type="checkbox" name="batch" id="batch" <?if($IS_BLACKLIST==1) echo "checked";?>></td>
    </tr>
    <tr style="display: none" id="BLACKLIST_INFO">
    	<td nowrap class="TableData"><?=_("黑名单说明：")?></td>
      <td class="TableData"colspan=3>
        <textarea name="BLACKLIST_INSTRUCTIONS" cols="70" rows="3" class="BigInput" ><?=$BLACKLIST_INSTRUCTIONS?></textarea>
      </td>
    </tr>
    <tr>
...
?>
    ...
    <tr align="center" class="TableControl">
      <td colspan=4 nowrap>
      	...
      	<input type="hidden" value="<?=$PAGE_FROM?>" name="PAGE_FROM">

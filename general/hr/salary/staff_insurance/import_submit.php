...
{	
	    ...
	    if($success==1)
	    {
            ...
            //echo $query2;
		  	...
		  	if(mysql_num_rows($cursor2)>0)
		  	{
		  		...
		  		$MSG_ERROR[$ROW_COUNT]=sprintf(_("员工%s的薪资基数修改完成！")."<br>", $USER_NAME);
		  		$UPDATE_COUNT++;
		  	}
		  	else
		  	{
		  		...
		  		exequery(TD::conn(),$query);
		  		$MSG_ERROR[$ROW_COUNT]=sprintf(_("员工%s的薪资基数导入完成！")."<br>", $USER_NAME);
		  		$INSERT_COUNT++;
		  	}
	  	}
	}
	...
	{
		$MSG_ERROR[$ROW_COUNT]="<font color=red><b>"._("薪资基数没有定义!")."</b></font><br>";
		break;
	}
	$ROW_COUNT++;
}//外层foreach

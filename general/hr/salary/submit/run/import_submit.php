...
$query = "SELECT ITEM_ID,ITEM_NAME from SAL_ITEM where ISREPORT='0' and ISCOMPUTER='0' order by ITEM_ID";
$cursor= exequery(TD::conn(),$query);
$title=array();
...
{   
	...
	else if($DATA['USER_ID']!="")
	{
        ...
        $user_info_id = $user_info_arr[$USER_ID]['USER_ID'];
	...
	if($USEFUL_COLUMN>0)
	{
		...
		if($YES_OTHER==1)
		{
	     ...
	     if($success==1)
	     {
		  	$query2 = "SELECT * from SAL_DATA where FLOW_ID='$FLOW_ID' and USER_ID='$user_info_id'";
		  	$cursor2=exequery(TD::conn(),$query2);
		  	if($ROW2=mysql_fetch_array($cursor2))
		  	{
		  		$query="update SAL_DATA set IS_FINA_INPUT='1',";
		  		$query.=$STR_UPDATE;
		  		$query.=" where FLOW_ID=$FLOW_ID and USER_ID='$user_info_id'"; 
		  	...
		  	{
		  		$query="insert into SAL_DATA(FLOW_ID,USER_ID,IS_FINA_INPUT,";
		  		$query.=$STR_KEY.") values ('$FLOW_ID','$user_info_id','1',".$STR_VALUE.")";
		  		exequery(TD::conn(),$query);

...
{
	$INSURANCE_ARR=array("PENSION_U","PENSION_P","MEDICAL_U","MEDICAL_P","FERTILITY_U","UNEMPLOYMENT_U","UNEMPLOYMENT_P","INJURIES_U","HOUSING_U","HOUSING_P"); 	
	$count=0;
	$count1=0;
	$length=0;
	...
	{ 
		...
		foreach($INSURANCE_ARR as $value)
		{
			$$value=$ROW[$value];
		}
		
		for($i=0;$i<count($Compute_Formula);$i++)
		{
			foreach($INSURANCE_ARR as $va)
			{
				if(strpos($Compute_Formula[$i],$va))
					$Compute_Formula[$i]=str_replace("[$".$va."]",$$va,$Compute_Formula[$i]);
			}
		}
		for($I=0;$I< $ARRAY_COUNT;$I++)
		{
		...
		for($I=0;$I< $ARRAY_COUNT;$I++)
		{
			...
			echo _("员工").$user_name._("的工资项目更新完成!!!")."<br>";
		} 
	}
	   ...
	   if (substr($temp[$I],0,strpos($temp[$I],"]"))!="")
	    {
	    	 ...
	    	 for($h=0;$h<$ARRAY_COUNT1;$h++)
	    	 {
	    	 	...
	    	 	if ($ARRAY_Sequence[$h]==$tempid || in_array($tempid,$INSURANCE_ARR)) 
	    	 		break;
	    	 }

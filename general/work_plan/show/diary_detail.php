...
$query = "SELECT NAME from WORK_PLAN where PLAN_ID='$PLAN_ID'";
$cursor=exequery(TD::conn(),$query);
if($ROW=mysql_fetch_array($cursor))
    $WORK_PLAN_NAME=$ROW["NAME"];

$HTML_PAGE_TITLE = _("进度日志详情");
...
<br>
<?
$query = "SELECT * from WORK_DETAIL where TYPE_FLAG='0'and PLAN_ID='$PLAN_ID' order by WRITE_TIME desc";
$cursor=exequery(TD::conn(),$query);
$DETAIL_COUNT=0;
    ...
    <tr class="<?=$TableLine?>">
        ...
        <td nowrap align="center">
            ...
            if($_SESSION["LOGIN_USER_ID"]==$WRITER or $_SESSION["LOGIN_USER_PRIV"]==1)
            {
                ...
                <a href="javascript:delete_diary('<?=$DETAIL_ID?>','<?=$PLAN_ID?>','<?=$ATTACHMENT_ID?>','<?=$ATTACHMENT_NAME?>');"> <?=_("删除")?></a>
                <?
            }
            ?>
        </td>

...
{
  window.location="add_diary.php?PLAN_ID=<?=$PLAN_ID?>&FIELD="+field+"&ASC_DESC="+asc_desc;
}
...
if($DETAIL_ID1=="")
{
   $query1 = "SELECT MAX(PERCENT) AS PERCENT_M from WORK_DETAIL where TYPE_FLAG='0' and WRITER='".$_SESSION["LOGIN_USER_ID"]."' and PLAN_ID='$PLAN_ID'";
...
else
{
   ...
   $query1 = "SELECT MAX(PERCENT) AS PERCENT_CM from WORK_DETAIL where TYPE_FLAG='0' and  WRITER='".$_SESSION["LOGIN_USER_ID"]."' and PLAN_ID='$PLAN_ID' and PERCENT < '$PERCENT_M'";
   $cursor1= exequery(TD::conn(),$query1);
   if($ROW1=mysql_fetch_array($cursor1))
...
{
?>
     	 <a href="edit_diary.php?DETAIL_ID=<?=$DETAIL_ID?>&PLAN_ID=<?=$PLAN_ID?>"> <?=_("修改")?></a>
       <a href="javascript:delete_diary('<?=$DETAIL_ID?>','<?=$PLAN_ID?>','<?=$ATTACHMENT_ID1?>','<?=$ATTACHMENT_NAME1?>');"> <?=_("删除")?></a>
<?
}
?>
     </td>

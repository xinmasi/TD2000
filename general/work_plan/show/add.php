...
if($DETAIL_ID1=="")
    $query="insert into WORK_DETAIL (PLAN_ID,WRITE_TIME,PROGRESS,PERCENT,TYPE_FLAG,WRITER,ATTACHMENT_ID,ATTACHMENT_NAME) values ('$PLAN_ID','$WRITE_TIME','$PROGRESS','$PERCENT','0','".$_SESSION["LOGIN_USER_ID"]."','$ATTACHMENT_ID','$ATTACHMENT_NAME')";
else
    $query="update WORK_DETAIL set PLAN_ID='$PLAN_ID',WRITE_TIME='$WRITE_TIME',PROGRESS='$PROGRESS',PERCENT='$PERCENT',TYPE_FLAG='0',WRITER='".$_SESSION["LOGIN_USER_ID"]."',ATTACHMENT_ID='$ATTACHMENT_ID',ATTACHMENT_NAME='$ATTACHMENT_NAME' where DETAIL_ID='$DETAIL_ID1'";
...
$sql2="update TASK set RATE='$PERCENT' where WORK_PLAN_ID='$PLAN_ID' and USER_ID='".$_SESSION["LOGIN_USER_ID"]."'";
...
if($WRITE_IN_WORK=="on")
{
    $query = "SELECT DIA_ID,CONTENT from DIARY where to_days(DIA_TIME)=to_days('$CUR_TIME') and DIA_TYPE='1' and USER_ID='".$_SESSION["LOGIN_USER_ID"]."' order by DIA_TIME desc limit 0,1";
...
if($OP!="0")
{
    ...
    $SMS_CONTENT=_("有新的工作计划进度日志，请查看。");

    $REMIND_URL="1:work_plan/show/diary_detail.php?PLAN_ID=".$PLAN_ID;
    if($SMS_REMIND=="on")
...
if($FLAG==1)
    $DETAIL_ID1="";
if($OP==0)
    header("location: add_diary.php?DETAIL_ID1=$DETAIL_ID1&PLAN_ID=$PLAN_ID");
else

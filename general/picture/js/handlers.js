...
function uploadProgress(fileObj, bytesLoaded) {
	...
	try {
		...
		if (percent === 100) {
			progress.SetStatus(td_lang.general.msg_1);//"上传成功"
			progress.ToggleCancel(false);
			progress.ToggleCancel(true, this, fileObj.id);
		} else {
			progress.SetStatus(td_lang.general.msg_2);//"正在上传..."
			progress.ToggleCancel(true, this, fileObj.id);
		}
	} catch (ex) { this.debug(ex); }
}
...
function uploadComplete(fileObj) {
	try {
		...
		if (this.getStats().files_queued > 0) {
			...
			progress.SetStatus(td_lang.general.msg_3);//"图片批量上传完毕."
			progress.ToggleCancel(false);
			location.reload();
		}
	} catch (ex) { this.debug(ex); }
		...
		switch(error_code) {
			case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED:
				try {
					...
					progress.SetStatus(td_lang.general.msg_4);//"停止"
					progress.ToggleCancel(true, this, fileObj.id);
				}
				catch (ex) { this.debug(ex); }

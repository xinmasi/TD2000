...
function picdelete(PIC_ID,SUB_DIR,PIC_PATH,LOCATION){
  if(TmpFileNameStr!="")
  {
     ...
     {
       URL="picdelete.php?DelFileNameStr="+escape(TmpFileNameStr)+"&PIC_ID="+PIC_ID+"&PIC_PATH="+PIC_PATH+"&LOCATION="+LOCATION+"&SUB_DIR="+SUB_DIR;
       window.location=URL;
     }
  }else{
...
function do_action(SUB_DIR,PIC_PATH)
{
  if(TmpFileNameStr!="")
  {
  	 URL="batch_down.php?TmpFileNameStr="+escape(TmpFileNameStr)+"&SUB_DIR="+SUB_DIR+"&PIC_PATH="+PIC_PATH;
     window.location=URL;
  }
  else
  {	  

...
{}*/
//echo $USER_ID."ERER";EXIT;
   
/*$query1 = "SELECT DEPT_ID from USER where USER_ID='$USER_ID'";
$cursor1= exequery(TD::conn(),$query1);
...
/*$query1 = "SELECT DEPT_ID from USER where UID='".$_SESSION["LOGIN_UID"]."'";
$cursor1= exequery(TD::conn(),$query1);
if($ROW1=mysql_fetch_array($cursor1))
   $DEPT_ID=$ROW1["DEPT_ID"];

//$query = "SELECT BOOK_ID,BOOK_NO,DEPT,OPEN,BOOK_NAME from BOOK_INFO where ((BOOK_NO like '%$BOOK_NO%') or (BOOK_NAME like '%$BOOK_NO%')) and (OPEN='0' or OPEN='1' or find_in_set('$DEPT_ID',OPEN) or OPEN='ALL_DEPT') order by BOOK_NO limit 0,50";
$query = "SELECT BOOK_ID,BOOK_NO,DEPT,OPEN,BOOK_NAME from BOOK_INFO where ((BOOK_NO like '%$BOOK_NO%') or (BOOK_NAME like '%$BOOK_NO%')) and (OPEN='0' or OPEN='1' or find_in_set('$DEPT_ID',OPEN) or OPEN='ALL_DEPT') order by BOOK_NO limit 0,50";
*/

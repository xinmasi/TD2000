...
if($ROW=mysql_fetch_array($cursor))
{
    ...
    //会议类型，视频多会议室模式传入23，培训多教室模式传入22
    $MEETING_AP=$ROW["MEETING_AP"];
    if($MEETING_AP == "22"){
        $MEETING_AP = _("培训会议模式");
    }else if($MEETING_AP == "23"){
        $MEETING_AP = _("视频会议模式");
    }
    $TOK=strtok($RECORDER,",");
...
<div id="main">
    ...
    <div class="meeting-content">
        ...
        <div class="meeting"><label class="meeting-font"><?=_("会议类型：")?></label><?=$M_TYPE_NAME?></div>
        <?if($M_TYPE == "1"){?>
            <div class="meeting"><label class="meeting-font"><?=_("会议模式：")?></label><?=$MEETING_AP?></div>
        <?}?>
    ...
    <div class="meeting-footer">
        <?=_("该会议由").$USER_NAME._("于  ").$M_REQUEST_TIME._(" 申请") ?>
    </div>
<?
}
else

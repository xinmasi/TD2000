...
{
 window.location="history.php?start=<?=$start?>&ASC_DESC="+ASC_DESC;
}
</script>
...
<table border="0" width="95%" cellspacing="0" cellpadding="3" class="small" align="center">
  <tr>
    <td class="Big"><img src="<?=MYOA_STATIC_SERVER?>/static/images/menu/vote.gif" align="absmiddle"><span class="big3"> <?=_("已终止的投票")?></span><br>
    </td>
  </tr>
</table>
<br>
...
<?
 //============================ 显示已终止公告 =======================================
 $VOTE_COUNT=0;
 $query = "SELECT * from VOTE_TITLE where PUBLISH='1' and PARENT_ID=0 and (TO_ID='ALL_DEPT' or find_in_set('".$_SESSION["LOGIN_DEPT_ID"]."',TO_ID)".dept_other_sql("TO_ID")." or find_in_set('".$_SESSION["LOGIN_USER_ID"]."',USER_ID) or find_in_set('".$_SESSION["LOGIN_USER_PRIV"]."',PRIV_ID)) and END_DATE!='0000-00-00' and END_DATE<='$CUR_DATE' order by TOP desc,BEGIN_DATE ".$order." limit $start,$PAGE_SIZE";
 $cursor= exequery(TD::conn(),$query);

...
function flow_trans(){
    window.open("/general/system/workflow/flow_guide/flow_type/set_trans/?1=1","set_trans","menubar=0,toolbar=0,scrollbars=no,status=0,resizable=1");
}

function set_batch(){
	window.open("/general/system/workflow/flow_guide/flow_type/set_all/?1=1","set_all","menubar=0,toolbar=0,scrollbars=no,status=0,resizable=1");
}
...
function update_cache(){
	ajax_update('/general/system/workflow/flow_guide/flow_type/update_cache.php');
}
...
function power_manage(flow_id)
{
	jQuery.ajax
	({		
		url: "/general/system/workflow/flow_guide/flow_type/set_manage/manage_data.php?FLOW_ID="+flow_id,
...
function manage_checkForm(TYPE){
	...
	{
        var manageadd = document.getElementById("manageadd");
        manageadd.disabled=true;
		var url = "/general/system/workflow/flow_guide/flow_type/set_manage/add.php";
	}
	else
	{
		var url = "/general/system/workflow/flow_guide/flow_type/set_manage/update.php";
	}
	jQuery.ajax
	({		
		url: url,
...
function set_timer(flow_id)
{
	jQuery.ajax
	({		
		url: "/general/system/workflow/flow_guide/flow_type/set_timer/timer_data.php?FLOW_ID="+flow_id,
		data: "FLOW_ID="+flow_id,
		type: "POST",
		async: true,
		dataType: 'json',
...
function timer_checkForm(flag)
{
	...
	document.getElementById("timer_new").action = "/general/system/workflow/flow_guide/flow_type/set_timer/save_schema.php?typetimer="+flag;
...
function set_query_templet(flow_id)
{
	jQuery.ajax
	({		
		url: "/general/system/workflow/flow_guide/flow_type/set_query_templet/index.php?FLOW_ID="+flow_id,
		data: "FLOW_ID="+flow_id,
		type: "POST",
		async: true,
		dataType: 'json',
...
function set_print_templet(flow_id)
{
	jQuery.ajax
	({		
		url: "/general/system/workflow/flow_guide/flow_type/set_print/print_data.php?FLOW_ID="+flow_id,
		data: "FLOW_ID="+flow_id,
		type: "POST",
		async: true,
		dataType: 'json',
...
function add_query_tpl(flow_id,seq_id)
{
	if(seq_id)
	{
		var URL = "/module/flow_query_tpl/new_search_modle.php?FLOW_ID="  + flow_id + "&SEQ_ID=" + seq_id;
	}else{
		var URL = "/module/flow_query_tpl/new_search_modle.php?FLOW_ID="  + flow_id;
	}
	width=(window.screen.availWidth-10);//设置打开的窗口的宽度
	height=(window.screen.availHeight-30);//设置打开的窗口的高度
	window.open (URL, 'flow_query_tpl', 'height='+height+', width='+width+', top=0, left=0, toolbar=no, menubar=no, scrollbars=yes, resizable=yes,location=no, status=no') ;
}
...
function convert()
{
	...
	_post("/general/system/workflow/flow_guide/flow_type/set_print/file_upload.php?FLOW_ID="+flow_id,"FLOW_ID="+flow_id+"&T_TYPE="+document.set_print.T_TYPE.value+"&T_NAME="+document.set_print.T_NAME.value+"&CONTENT="+content.replace(/\+/g, '%2B'),function(req){
		var t_id = req.responseText;
		var url = '/general/system/workflow/flow_guide/flow_type/set_print/new_tpl.php?T_ID='+t_id;
...
{	
	...
	arr_str[0]=td_lang.general.workflow.msg_manage_priv_type_0;
	arr_str[1]=td_lang.general.workflow.msg_manage_priv_type_1;
	arr_str[2]=td_lang.general.workflow.msg_manage_priv_type_2;
	arr_str[3]=td_lang.general.workflow.msg_manage_priv_type_3;
	arr_str[4]=td_lang.general.workflow.msg_manage_priv_type_4;
	arr_str[5]=td_lang.general.workflow.msg_manage_priv_type_5;
	if(obj == 1)
	{
...
function view_list_openURL()
{
	...
	G.openURL('',td_lang.system.workflow.msg_30,'/general/system/workflow/flow_guide/flow_type/flow_design/index.php?FLOW_ID='+flow_id+'&reloadFlag=1')
}

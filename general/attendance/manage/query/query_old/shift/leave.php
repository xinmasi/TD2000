...
$query = "SELECT DEPT_NO,USER_NO,USER_NAME,LEAVE_TYPE2,USER.DEPT_ID,USER.USER_ID from ATTEND_LEAVE,USER,USER_EXT,DEPARTMENT where DEPARTMENT.DEPT_ID=USER.DEPT_ID and USER_EXT.USER_ID=USER.USER_ID and USER_EXT.DUTY_TYPE='99' and ATTEND_LEAVE.USER_ID=USER.USER_ID and ((to_days(LEAVE_DATE1)>=to_days('$DATE1') and to_days(LEAVE_DATE1)<=to_days('$DATE2')) or (to_days(LEAVE_DATE2)>=to_days('$DATE1') and to_days(LEAVE_DATE2)<=to_days('$DATE2')) or (to_days(LEAVE_DATE1)<=to_days('$DATE1') and to_days(LEAVE_DATE2)>=to_days('$DATE2'))) and allow in('1','3') group by DEPT_NO,USER_NO,USER_NAME,LEAVE_TYPE2 order by DEPT_NO,USER_NO,USER_NAME,LEAVE_TYPE2";
$cursor= exequery(TD::conn(),$query, $connstatus);
$LEAVE_COUNT=0;
$USER_COUNT=0;
$LEAVE_DAYS=0;
while($ROW=mysql_fetch_array($cursor))
{
   ...
   $query_extra = "SELECT * from ATTEND_LEAVE,USER,DEPARTMENT,USER_EXT where DEPARTMENT.DEPT_ID=USER.DEPT_ID and USER.UID=USER_EXT.UID and USER_EXT.DUTY_TYPE='99' and ATTEND_LEAVE.USER_ID=USER.USER_ID and ((to_days(LEAVE_DATE1)>=to_days('$DATE1') and to_days(LEAVE_DATE1)<=to_days('$DATE2')) or (to_days(LEAVE_DATE2)>=to_days('$DATE1') and to_days(LEAVE_DATE2)<=to_days('$DATE2')) or (to_days(LEAVE_DATE1)<=to_days('$DATE1') and to_days(LEAVE_DATE2)>=to_days('$DATE2'))) and allow in('1','3') and USER.DEPT_ID='$DEPT_ID' and USER.USER_ID='$USER_ID' and LEAVE_TYPE2 ='$LEAVE_TYPE2'";
   $cursor_extra= exequery(TD::conn(),$query_extra, $connstatus);
   ...
   $query5 = "SELECT * from ATTEND_LEAVE,USER,DEPARTMENT,USER_EXT where DEPARTMENT.DEPT_ID=USER.DEPT_ID and USER.UID=USER_EXT.UID and USER_EXT.DUTY_TYPE='99' and ATTEND_LEAVE.USER_ID=USER.USER_ID and ((to_days(LEAVE_DATE1)>=to_days('$DATE1') and to_days(LEAVE_DATE1)<=to_days('$DATE2')) or (to_days(LEAVE_DATE2)>=to_days('$DATE1') and to_days(LEAVE_DATE2)<=to_days('$DATE2')) or (to_days(LEAVE_DATE1)<=to_days('$DATE1') and to_days(LEAVE_DATE2)>=to_days('$DATE2'))) and allow in('1','3') and USER.DEPT_ID='$DEPT_ID' and USER.USER_ID='$USER_ID' and LEAVE_TYPE2 ='$LEAVE_TYPE2'";
   $cursor5= exequery(TD::conn(),$query5, $connstatus);
...
<?
      ...
      {
?>
         <a href="delete.php?MYACTION=leave&LEAVE_ID=<?=$LEAVE_ID?>&DEPARTMENT1=<?=$DEPARTMENT1?>&DUTY_TYPE=<?=$DUTY_TYPE?>&DATE1=<?=$DATE1?>&DATE2=<?=$DATE2?>"><?=_("删除")?></a>
<?
      }
?>
         </td>

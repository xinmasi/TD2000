...
$objExcel->setFileName(_("分组联系人"));
$objExcel->addHead($OUTPUT_HEAD);

$query = "SELECT * from ADDRESS where (USER_ID='".$_SESSION["LOGIN_USER_ID"]."' or (find_in_set('".$_SESSION["LOGIN_USER_ID"]."',SHARE_USER) and (ADD_START='0000-00-00 00:00:00' or ADD_START<='$TIME') and (ADD_END='0000-00-00 00:00:00' or ADD_END>='$TIME'))) and find_in_set(GROUP_ID,'$export_group') order by GROUP_ID";
$cursor= exequery(TD::conn(),$query);
while($ROW = mysql_fetch_array($cursor))
{
    $ADD_ID          = $ROW["ADD_ID"];

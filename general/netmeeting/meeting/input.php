...
{    
   ...
   var meet_id=document.form1.MEET_ID.value;
   var to_name = document.form1.TO_NAME.value;
   var user_name=document.form1.USER_NAME.value;
	var url="input.php";
	var postStr="MESSAGE="+message+"&MEET_ID="+meet_id+"&TO_NAME="+to_name+"&USER_NAME="+user_name+"&QUIET="+quiet+"&TO_ID="+to_id+"&COLOR="+color;
	var ajax = false;
	url=encodeURI(url); 
	url=encodeURI(url); 
   ...
   if (!ajax)
   {
      window.alert("<?=_("不能创建XMLHttpRequest对象实例")?>.");
     	return false;
   }
...
<table class="TableBlock" width="100%" align="center">
  ...
  <tr class="TableHeader">
     <td>
        <?=_("内容")?>
        <input type="text" name="MESSAGE" size="80" class="smallInput" onkeydown="if(event.keyCode==13){testhasSubmit();return false;}">
        <input type="button" value="<?=_("发言")?>" class="smallButton" onclick="testhasSubmit()">&nbsp;&nbsp;&nbsp;
        <a href="#" onclick=window.open('history.php?MEET_ID=<?=$MEET_ID?>','oa_sub_window','height=500,width=600,status=0,toolbar=no,menubar=no,location=no,left=300,top=200,scrollbars=yes,resizable=yes')><b><font size='2'><?=_("查看历史记录")?></font></b></a>
...
$MEET_ID=iconv("UTF-8",MYOA_CHARSET,$MEET_ID);
$MSG=str_replace("\n"," ",$MESSAGE);

...
<tbody>
	...
	<?php
		$query = "select * from proj_budget_real where status = '0,0,0,' and proj_id = '$PROJ_ID' and real_id = '" . $LOGIN_USER_ID . "'";
		//项目负责人经理创建者可以查看全部资金申请
		if(project_view_priv($PROJ_ID) == 2){
			$query = "select * from proj_budget_real where status = '0,0,0,' and proj_id = '$PROJ_ID'";
		}
...
var unp = false;
function unpass(un,step){
    if(unp)
        unp.close();
    URL = "budget_record/budget.php?unpass="+un+"&step="+step+"&PROJ_ID=<?=_($PROJ_ID)?>";
    unp = window.open (URL, "_blank", "height=400, width=500, toolbar =no, menubar=no, scrollbars=no, resizable=no, location=no, status=no,top=150,left=200");
}

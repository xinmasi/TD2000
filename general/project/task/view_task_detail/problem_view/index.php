...
while($ROW=mysql_Fetch_array($cursor))
{
  ...
  <tr class="TableHeader">
	...
	echo '<tr class="'.$TableLine.'"> 
	  ...
	  <td nowrap align="center"><span class="CalLevel'.$ROW["LEVEL"].'" title="'.level_desc($ROW["LEVEL"]).'">&nbsp;</span></td>
	  <td nowrap align="center">'.$STATUS_DESC.'</td>
	  <td nowrap align="center">';
  if($ROW["STATUS"]==0)	
     echo '<a href="#" onclick=update("'.$ROW["BUG_ID"].'")>'._("提交").'</a>&nbsp;<a href="new.php?BUG_ID='.$ROW["BUG_ID"].'&PROJ_ID='.$PROJ_ID.'&TASK_ID='.$TASK_ID.'">'._("修改").'</a>&nbsp;<a href="#" onclick=delete_bug("'.$ROW["BUG_ID"].'") >'._("删除").'</a>&nbsp;';
  elseif($ROW["STATUS"]==3)
     echo '<a href="#" onclick=goback("'.$ROW["BUG_ID"].'")>'._("退回").'</a>';   
  else
     echo "<font color=green>"._("已提交")."</font>";
  echo '</td></tr>';
}
?>	
</table>
<?
if($COUNT==0)
   Message(_("提示"),_("没有已提交问题！"));
?>
<div id="overlay"></div>
<div id="detail" class="ModalDialog" style="width:550px;">
  <div class="header"><span id="title" class="title"><?=_("项目问题详情")?></span><a class="operation" style="padding-top:2px" href="javascript:HideDialog('detail');"><img src="<?=MYOA_STATIC_SERVER?>/static/images/close.png"/></a></div>
...
<div id="back" class="ModalDialog" style="width:550px;">
  <div class="header"><span id="title" class="title"><?=_("项目问题退回")?></span><a class="operation" style="padding-top:2px" href="javascript:HideDialog('back');"><img src="<?=MYOA_STATIC_SERVER?>/static/images/close.png"/></a></div>

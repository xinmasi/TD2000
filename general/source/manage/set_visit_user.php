...
function set_view_type()
{
	...
	if(value==2)
        ...
        location="set_visit_user.php?SOURCEID=<?=$SOURCEID?>&VIEW_TYPE="+value;
}

function set_view_name()

{
	var name=document.getElementById("NAME").value;		
	location="set_visit_user.php?SOURCEID=<?=$SOURCEID?>&VIEW_TYPE=2&NAME="+name;
}
</script>


<body class="bodycolor" onLoad="func_init();">
<form action="visit_user_submit.php" method="post" name="form1">
<?
$query = "select VISIT_USER from OA_SOURCE where SOURCEID='$SOURCEID'";
...
<table border="0" width="100%" cellspacing="0" cellpadding="3" class="small">
  <tr>
    <td class="Big"><img src="<?=MYOA_STATIC_SERVER?>/static/images/edit.gif" WIDTH="22" HEIGHT="20" align="absmiddle"><span class="big3"> <?=_("指定资源申请用户权限")?></span>
    &nbsp;
    <select class="BigSelect" name="VIEW_TYPE" onChange="set_view_type();" id="selected">
    <option value="0"><?=_("按部门选择人员")?></option>
    <option value="1" <?if($VIEW_TYPE==1)echo "selected";?>><?=_("按角色选择人员")?></option>
    <option value="2" <? if ($VIEW_TYPE==2) echo "selected";?>><?=_("按搜索人名选择人员")?></option>
    </select>&nbsp;
    <span id="search" style="display:<?if ($VIEW_TYPE!=2) echo "none"?>">
           <?=_("请输入搜索内容")?><input type="text" name="" id="NAME" value="<?=$NAME?>" onChange="set_view_name();">
    </span>
    </td>
  </tr>
</table>
  ...
  <tr>
    ...
    <select  name="select1" ondblclick="func_delete();" MULTIPLE style="width:200px;height:200px;">
      <?
        ...
        {
            $dept_str = get_manage_dept_ids($_SESSION['LOGIN_UID']);
            if($dept_str)
            {
                $dept_ids = $dept_str;
            }
        ...
        $query = "SELECT USER.USER_ID,USER_NAME,DEPARTMENT.DEPT_ID,USER_PRIV.USER_PRIV from USER,USER_PRIV,DEPARTMENT ".$WHERE_STR." and  USER.DEPT_ID=DEPARTMENT.DEPT_ID and USER.USER_PRIV=USER_PRIV.USER_PRIV and (USER.NOT_LOGIN='0' or USER.NOT_MOBILE_LOGIN='0')";
        if($VIEW_TYPE==0)
           $query.= " order by DEPT_NO,USER.DEPT_ID,PRIV_NO,USER_NO,USER_NAME";
        else if ($VIEW_TYPE==1)
           $query.= " order by PRIV_NO,DEPT_NO,USER_NO,USER_NAME";
        else if ($VIEW_TYPE==2)
           $query.= "order by USER_ID,USER_NAME";  
        ...
        while($ROW=mysql_fetch_array($cursor))
        {
          ...
          if($VIEW_TYPE==0)
          {
            $query1 = "SELECT DEPT_NAME from DEPARTMENT where DEPT_ID=" .$DEPT_ID1;
      ...
      <?
        ...
        if($VIEW_TYPE==0)
           ...
           $query.= " order by PRIV_NO,DEPT_NO,USER_NO,USER_NAME";
            ...
            $query="select USER.USER_ID,USER_NAME,DEPARTMENT.DEPT_ID,USER_PRIV.USER_PRIV FROM USER,USER_PRIV,DEPARTMENT ".$WHERE_STR." and  USER.DEPT_ID=DEPARTMENT.DEPT_ID and USER.USER_PRIV=USER_PRIV.USER_PRIV and (USER.NOT_LOGIN='0' or USER.NOT_MOBILE_LOGIN='0') and (USER_NAME like '%$NAME%' or USER_ID like '%$NAME%')";
            $cursor=exequery(TD::conn(),$query);
        while($ROW=mysql_fetch_array($cursor))
        {
          $USER_ID=$ROW["USER_ID"];

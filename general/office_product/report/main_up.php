...
<table  style="text-align: center;" class="table table-bordered query-table " border="0" style=" margin-top: 10px; ">
    <tr>
        <td nowrap  width="10%"><?=_("办公用品库")?><span style="color: red;padding-left: 1px;"></span> </td>
        <td nowrap width = "20%">
...
?>
        ...
        <td nowrap width = "10%"><?=_("办公用品类别")?><span style="color: red;padding-left: 1px;"></span></td>
        ...
        <td nowrap   width = "10%"><?=_("办公用品")?><span style="color: red;padding-left: 1px;"></span></td>
...
function selectData(url,trans_flag)
{
    ...
    var OFFICE_DEPOSITORY = document.form1.OFFICE_DEPOSITORY.value == -1 ? '' : "&OFFICE_DEPOSITORY="+document.form1.OFFICE_DEPOSITORY.value;
    var OFFICE_PROTYPE = document.form1.OFFICE_PROTYPE.value == -1 ? '' : "&OFFICE_PROTYPE="+document.form1.OFFICE_PROTYPE.value;
    var PRO_ID = document.form1.PRO_ID.value == -1 ? '' : "&PRO_ID="+document.form1.PRO_ID.value;
    if(url == 'purchase.php')
    {
        if(MAP_VALUE==0 || MAP_VALUE==1)
        {
            ...
            URL = url+"?FROM_DATE="+document.form1.FROM_DATE.value+"&TO_DATE="+document.form1.TO_DATE.value+PRO_ID+"&TRANS_FLAG="+trans_flag+OFFICE_PROTYPE+OFFICE_DEPOSITORY+'&MAP_TYPE='+MAP_VALUE;
        }
        else if(MAP_VALUE == 2)
        {
            url = 'report.php';
    ...
    {
        URL = url+"?FROM_DATE="+document.form1.FROM_DATE.value+"&TO_DATE="+document.form1.TO_DATE.value+PRO_ID+"&TRANS_FLAG="+trans_flag+OFFICE_PROTYPE+OFFICE_DEPOSITORY+'&MAP_TYPE='+MAP_VALUE+'&TO_ID1='+document.form1.TO_ID1.value;
    }
    else
    {
        var to_date=jQuery('#sure_date').val();
        URL = url+"?FROM_DATE="+document.form1.FROM_DATE.value+"&TO_DATE="+document.form1.TO_DATE.value+"&TRANS_FLAG="+PRO_ID+trans_flag+OFFICE_PROTYPE+OFFICE_DEPOSITORY;    //查询物品总表的时候少一个参数 ---照熙
    }
    if(document.form1.FROM_DATE.value!="" && document.form1.TO_DATE.value!="")
    {
        if(document.form1.FROM_DATE.value > document.form1.TO_DATE.value)
        {

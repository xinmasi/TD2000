...
if($LIST_TREE=='')
{
    ...
    while($ROW=mysql_fetch_array($cursor))
    {
        ...
        {
            if((find_id($ROW['DEPT_ID'],$_SESSION["LOGIN_DEPT_ID"]) || $ROW['DEPT_ID'] == '' || $ROW['DEPT_ID'] == 'ALL_DEPT' )&& (find_id($ROW['MANAGER'],$_SESSION["LOGIN_USER_ID"])))
            {
...
}elseif($LIST_TREE=='TYPE'&&$ID!="")
{
    ...
    while($ROW_TYPE=mysql_fetch_array($cursor_type))
    {
        ...
        if($ROW_COUNT_PRODUCTS[0] > 0){
            ...
            $url =  "list.php?DEPOSITORY=".$ID."&TYPE=".$ROW_TYPE['ID']."&DEPOSITORY_ID=".$TYPE_ID;
            $isLazy = true;
        }else{
            $json = "";
            $url = "list.php?DEPOSITORY=".$ID."&TYPE=".$ROW_TYPE['ID']."&DEPOSITORY_ID=".$TYPE_ID;
...
elseif($LIST_TREE=='PRODUCTS')
{
    ...
    while($ROW_PRODUCTS = mysql_fetch_array($cursor_products))
    {
        ...
        $url = "edit.php?DEPOSITORY=".$DEPOSITORY."&DEPOSITORY_ID=".$DEPOSITORY_ID."&TYPE=".$TYPE."&PRO_ID=".$ROW_PRODUCTS['PRO_ID'];

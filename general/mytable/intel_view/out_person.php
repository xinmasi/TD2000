...
while($ROW1=mysql_fetch_array($cursor))
{
   ...
   if($COUNT==1)
      echo '<fieldset class="small"><legend align=left><b>'._("出差人员").'</b></legend>';

   echo '<u title="'.sprintf(_("%s 至 %s"),$EVECTION_DATE1,$EVECTION_DATE2).' '._("地点：").$EVECTION_DEST._("事由：").$REASON.'" style="cursor:hand">'.$USER_NAME.'</u>&nbsp;';
}
...
while($ROW1=mysql_fetch_array($cursor))
{
   ...
   if($COUNT1==1)
      echo '<fieldset class="small"><legend align=left><b>'._("请假人员").'</b></legend>';

   echo '<u title="'.sprintf(_("%s 至 %s"),$LEAVE_DATE1,$LEAVE_DATE2)._("原因：").$LEAVE_TYPE.'" style="cursor:hand">'.$USER_NAME.'</u>&nbsp;';
}
...
while($ROW1=mysql_fetch_array($cursor))
{
   ...
   if($COUNT2==1)
      echo '<fieldset class="small"><legend align=left><b>'._("外出人员").'</b></legend>';
   echo '<u title="'.sprintf(_("%s 至 %s"),$OUT_TIME1,$OUT_TIME2).' '._("原因：").$OUT_TYPE.'" style="cursor:hand">'.$USER_NAME.'</u>&nbsp';
}

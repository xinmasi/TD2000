...
function showContextMenu(event,type)
{
  ...
  case 1:
  	...
  	sMenu += getMenuRow("Edit_Process("+Process_ID+")", td_lang.general.workflow.msg_21);//"步骤基本属性"
  	height += 20;

  	sMenu += getMenuRow("set_item("+Process_ID+")", td_lang.general.workflow.msg_23);//"可写字段"
  	height += 20;

  	sMenu += getMenuRow("set_user("+Process_ID+")", td_lang.general.workflow.msg_22);//"经办人员"
  	height += 20;

  	sMenu += getMenuRow("set_dept("+Process_ID+")", td_lang.general.workflow.msg_24);//"经办部门"
  	height += 20;

  	sMenu += getMenuRow("set_priv("+Process_ID+")", td_lang.general.workflow.msg_25);//"经办角色"
  	height += 20;

  	sMenu += sMenuHr;
  	height += 2;

  	sMenu += getMenuRow("Del_Process("+Process_ID+")", td_lang.general.workflow.msg_27);//"删除该步骤"
  	height += 20;
  ...
  case 2:
  	...
  	sMenu += getMenuRow("Add_Process()", td_lang.general.workflow.msg_28);//"新建步骤"
  	height += 20;

  	sMenu += sMenuHr;
  	height += 2;

  	sMenu += getMenuRow("SavePosition()", td_lang.general.workflow.msg_29);//"保存布局"
  	height += 20;

  	sMenu += getMenuRow("Refresh()", td_lang.general.workflow.msg_30);//"刷新视图"
  	height += 20;
...
/*
  case 3:
    var fid =  objRect.getAttribute('mfrID');
		sMenu += getMenuRow("DelFlowRun(" + fid + ")",  td_lang.general.workflow.msg_26);//"删除连线"
	  height += 20;
	  break;
*/
  }
...
{
	 msg=td_lang.general.workflow.msg_20;//'确认要删除该步骤么？'

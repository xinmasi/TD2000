        ...
        itemClickHandle: function(item, callbacks){
        	...
        	if(item.get('index') == 'new' || this.register.edit_op==1){
	            ...
	            $close.click(function(){ 
	                ...
	                if($('#content .tag-RListField').length == 0){
	                	self.$emptyTips.show();
            			self.$sumArea.hide();
	                }
	            });
...
<div class="read_detail clearfix tag-<%=fieldtype%>">\
  ...
  <div class="field">\
    ' + (cfg.editable == 1 ?
      '<input value="<%=colvalue%>" name="<%=id%>" type="text" style="font-size:15px; width:50%;" />'
    : '<span><%=colvalue%></span><input value="<%=colvalue%>" name="<%=id%>" type="hidden" />') +
  '</div>\
</div>\
';
...
define('RListField', function (require, exports, module) {
    ...
    var RListField = RTextField.extend({
        ...
        bindEvent: function(){
            var self = this;
            this.$el.delegate('.read_detail_delete', 'tap click', function(e){
                e.stopPropagation();
                var delflag=confirm(confirmDelete); 
                if (delflag == true){ 
                    self.trigger('delete', self);
                    saveFlag = 0;
                    if($('#scroller_18').find('.read_detail').length == 0){
                        $('#scroller_18').find('#emptyTips').css('display', 'block');
                    }
                    return true; 
                } 
                else{
        ...
        parseTpl: function (cfg) {
            ...
            var tmpl = (function(){
                ...
                $.each(cfg.item, function(i, v){
                    if(i == 0){
                        fields.push('<h6 class="clearfix listview-title"><b class="no">'+(cfg.index)+'</b><em class="colnamearea"><span>' + v.colname + ': </span><span>' + emptyHelper(v.colvalue) + '</span></em></h6>');
                    }else{
                        fields.push('<em>' + v.colname + ': ' + emptyHelper(v.colvalue,v) + '</em>');
                    }
                });
                var newClass = (cfg.index == 'new')? 'list-item-new':'';
                return '<div class="read_detail clearfix tag-RListField"><div class="clearfix list-item '+ newClass +'">'+ fields.join('\n\r') +'</div><div class="read_detail_delete"><span>'+ read_detail_delete +'</span></div></div>';
            })();
            return $.parseTpl(tmpl, cfg)
        },
        render: function(){
...
';
        ...
        renderItem: function(cfg){
            ...
            $.each(opData, function (k, v) {
                ...
                html.push('<input type="checkbox" name="'+cfg.id+'" id="'+ cfg.id+'_'+k +'" ' + checked + ' /><label style="margin:0px 3px;" for="'+ cfg.id+'_'+k +'">'+ this +'</label>');
            });
            this.$field.append(html.join(''));
        },
        initialized: function(){ 
...
';
        ...
        renderItem: function(cfg){
            ...
            $.each(opData, function (k, v) {
                ...
                html.push('<input type="radio" name="'+cfg.id+'" id="'+ cfg.id+'_'+k +'" ' + checked + ' /><label style="margin:0px 3px;" for="'+ cfg.id+'_'+k +'">'+ this +'</label>');
            });
            this.$field.append(html.join(''));
        },
        initialized: function(){

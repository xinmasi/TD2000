...
Ganttalendar.prototype.drawTask = function (task) {
  ...
  var top = editorRow.position().top+self.master.editor.element.parent().scrollTop();//图标的Y轴坐标
  var x = Math.round((task.start - self.startMillis) * self.fx);  //图标的X轴坐标
...
Ganttalendar.prototype.drawLink = function (from, to, type) {
  ...
  function drawStartToEnd(rectFrom, rectTo, peduncolusSize) {
    ...
    //arrow
    var arr = $("<img src='edit/jQueryGantt-master/linkArrow.png'>").css({
      position: 'absolute',
...
Ganttalendar.prototype.centerOnToday = function() {
  ...
  this.element.parent().scrollLeft(x-150);
};

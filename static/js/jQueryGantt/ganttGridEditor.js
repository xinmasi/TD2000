...
GridEditor.prototype.fillEmptyLines = function() {
  ...
  var rowsToAdd = 9 - this.element.find(".taskEditRow").size();
...
GridEditor.prototype.refreshTaskRow = function(task) {
  ...
  row.find("[name=zxr]").html(task.zxr);
...
GridEditor.prototype.openFullEditor = function (task, taskRow) {
  ...
  taskEditor.find("#status").attr("status", task.status);
	...
	taskEditor.find("#bz").val(task.bz);
	taskEditor.find("#zxr").val(task.zxr);
	taskEditor.find("#sjrw").val(task.sjrw);
	taskEditor.find("#qzrw").val(task.qzrw);
	taskEditor.find("#rwjb").val(task.rwjb);
	//taskEditor.find("#lcb").is(":checked");
	taskEditor.find("#glgzl").val(task.glgzl);
  ...
  if (!self.master.canWrite) {
    ...
    taskEditor.find("#saveButton").click(function () {
	  ...
	  task.bz=taskEditor.find("#bz").val();
	  task.zxr=taskEditor.find("#zxr").val();
	  task.sjrw=taskEditor.find("#sjrw").val();
	  task.qzrw=taskEditor.find("#qzrw").val();
	  task.rwjb=taskEditor.find("#rwjb").val();
	//  task.lcb=taskEditor.find("#lcb").is(":checked");
	  task.glgzl=taskEditor.find("#glgzl").val();

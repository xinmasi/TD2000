...
{
   var Main_Tab = document.getElementById(tableName);
   if(!Main_Tab) return;
...
function sort_tab(the_tab,col,mode)
{
   ...
   for(i=1;i<the_tab.rows.length;i++)
   {
      ...
      if(the_tab.rows[i].tagName.toLowerCase()=="tfoot" || the_tab.rows[i].className.toLowerCase()=="tablecontrol")
         tfoot = the_tab.rows[i];
      else
   ...
   if(tfoot)
      the_tab.lastChild.appendChild(tfoot);
}

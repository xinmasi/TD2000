                                          ...
                                          success: function (msg) {
                                                if (msg.status == 0) {
                                                      ...
                                                      jQuery.each(msg.data, function (index, element) {
                                                            ...
                                                            if (userSaveAndBind.regexpCheck(element.uid).test(userSaveAndBind[jQuery(".alert-modal-box").data("to-id")].toString() + ",")) {
																  //请问这段是谁写的，为什么处理jQuery("#myModal").length？svn由于建分支没有记录了。--tianlin
																  /*
																  ...
																  */
																  //注释以上代码是修复业务流程平台转交增加传阅人时人可以重复选的问题 --tianlin
                                                ...
                                                if (msg.status == 0) {
													...
													//console.log('success',msg)
                                                      ...
                                                      jQuery.each(msg.data, function (index, element) {
                                                            ...
                                                            if (userSaveAndBind.regexpCheck(element.user_id).test(userSaveAndBind[jQuery(".alert-modal-box").data("to-id")].toString() + ",")) {
																  //请问这段是谁写的，为什么处理jQuery("#myModal").length？svn由于建分支没有记录了。--tianlin
																  /*
                                                                  if (jQuery("#myModal").length > 0) {
                                                                        return;
                                    ...
                                    success: function (msg) {
                                          if (msg.status == 0) {
                                                ...
                                                jQuery.each(msg.data, function (index, element) {
                                                      ...
                                                      if (userSaveAndBind.regexpCheck(element.dept_id).test(userSaveAndBind[jQuery(".alert-modal-box").data("to-id")].toString() + ",")) {
                                          ...
                                          if (msg.status == 0) {
                                                ...
                                                jQuery.each(msg.data, function (index, element) {
                                                      if (element.user_priv == undefined) {
                                                            ...
                                                            if (userSaveAndBind.regexpCheck(element.user_priv).test(userSaveAndBind[jQuery(".alert-modal-box").data("to-id")].toString() + ",")) {
      ...
      if (jQuery("#dept_item_children_" + node.data.dept_id).length > 0) {
            ...
            if (select == true) {
                  jQuery.each(jQuery("#dept_item_children_" + node.data.dept_id + " .block-right-item"), function (index, element) {
                        ...
                        if (userSaveAndBind.regexpCheck(jQuery(element).attr("item_id")).test(userSaveAndBind[jQuery(".alert-modal-box").data("to-id")].toString() + ",")) {
                              return;
                        } else {
                                    ...
                                    if (msg.status == 0) {
                                          ...
                                          jQuery.each(msg.data, function (index, element) {
                                                ...
                                                if (element.user_priv == undefined) {
                                                      ...
                                                      jQuery("#dept_item #dept_item_children_" + node.data.dept_id).append("<div class='block-right-item' item_id=" + element.user_priv + " item_name=" + element.priv_name.replace(/\s/g, '') + " user_id=" + element.user_priv + ">" + element.priv_name + "</div>")

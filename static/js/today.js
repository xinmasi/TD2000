...
function _del(a)
{
	msg=td_lang.inc.msg_91;//"确认不显示该模块么?"
...
function SetNums()
{
   ...
   {
      alert(td_lang.inc.msg_92);//"请输入数值"
      return;
   }
   if(checkNum(today_lines) || checkNum(today_time_out))
   {
      alert(td_lang.inc.msg_93);//"显示条数和连接超时必须是数字"
      return;
   }

   if(parseInt(today_lines)<=0 || parseInt(today_lines)>1000)
   {
      alert(td_lang.inc.msg_94);//"显示行数必须在1-1000之间"
      return;
   }
...
function add_module(url_id,url,url_desc)
{
   if($("module_"+url_id))
   {   
   	if($("module_"+url_id).style.display=="none")
   	{
   		$("module_"+url_id).style.display='block';
      	alert(td_lang.inc.msg_142);//"该模块已填加"
      	return;      
   	} 	
   	else
   	{
	     	$("module_"+url_id).style.display='block';
	      alert(td_lang.inc.msg_95);//"该模块已存在"
	      return;  
      }    
   }
   ...
   var req=getXMLHttpObj();
	...
	req.onreadystatechange=function()
	{
		...
		if(req.readyState==4)
		{
		   ...
		   if(module_ul) module_ul.innerHTML="<img src='/static/images/loading.gif' height='20' width='20' align='absMiddle'> 加载中，请稍候……";
	};
...
function abortRequest(req,module_id)
{
   ...
   var module_ul=$("module_"+module_id+"_ul");
   if(module_ul) module_ul.innerHTML=td_lang.inc.msg_96;//"操作超时"
}
function GetWeather(HOURS)
{
   var WEATHER_CITY = $('w_county').options[$('w_county').selectedIndex].value;
   if(WEATHER_CITY.length != 6)//"选择城市"
   {
      alert(td_lang.inc.msg_98);//"请选择城市"
      return;
   }
   ...
   $('weather').innerHTML="<img src='/static/images/loading.gif' height='20' width='20' align='absMiddle'> "+td_lang.inc.msg_43;//加载中，请稍候……
   var httpReq=getXMLHttpObj();
   httpReq.open("GET","/inc/weather.php?WEATHER_CITY="+ encodeURIComponent(WEATHER_CITY)+"&VIEW=b&HOURS="+HOURS,true);
   httpReq.onreadystatechange=function(){
      if(httpReq.readyState==4){
         if(httpReq.responseText=="error")
            $('weather').innerHTML=td_lang.inc.msg_99+" <a href=\"javascript:GetWeather();\">"+td_lang.global.refresh+"</a> <a href=\"#\" onclick=\"$('area_select').style.display='block';$('weather').style.display='none';\">"+td_lang.inc.msg_33+"</a>";//更改城市
         else
...
function NextDay(mdule_id, page)
{
   var o=$(mdule_id);
   if(!o)   return;
   
   o.scrollLeft+=page*152;
   if(o.scrollLeft > 0)
      $('module_'+mdule_id+'_link_pre').className="";
   else
      $('module_'+mdule_id+'_link_pre').className="PageLinkDisable";
   if(o.scrollWidth-o.scrollLeft>152)
      $('module_'+mdule_id+'_link_next').className="";
   else
      $('module_'+mdule_id+'_link_next').className="PageLinkDisable";
}
function my_radio(MY_URL,DESC_STR)
{
  ...
  URL="mytable/intel_view/my_radio.php?MY_URL="+MY_URL+"&DESC_STR="+DESC_STR;
...
function my_radio_fromportal(MY_URL,DESC_STR)
{
  mytop=screen.availHeight-160;
  myleft=screen.availWidth-420;;
  URL="../../mytable/intel_view/my_radio.php?MY_URL="+MY_URL+"&DESC_STR="+DESC_STR;
...
function my_tv_fromportal(MY_URL,DESC_STR)
{
  mytop=screen.availHeight-440;
  myleft=screen.availWidth-420;;
  URL="../../mytable/intel_view/my_radio.php?MY_URL="+MY_URL+"&IS_TV=1&DESC_STR="+DESC_STR;
  window.open(URL,"my_radio","height=400,width=400,right=0,top="+mytop+",left="+myleft+",status=0,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=yes");
}
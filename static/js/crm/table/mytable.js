...
{
	if(!$(module_id)) return;
	if($(module_id+"_head").scrollWidth - $(module_id+"_text").scrollWidth - 120 - 40 > 0)
	   $(module_id+"_title").style.width=$(module_id+"_head").scrollWidth - $(module_id+"_text").scrollWidth - 120 - 40 + "px";
	if($(module_id+"_head").scrollWidth - $(module_id+"_text").scrollWidth - 120 - 40 > 0)
	   $(module_id+"_title").style.width=$(module_id+"_head").scrollWidth - $(module_id+"_text").scrollWidth - 120 - 40 + "px";
	
	if(!$(module_id+"_type_link")) return;
	$(module_id+"_type_link").style.width = $(module_id+"_head").scrollWidth - 60 + "px";
}
...
function aI()
{
	...
	_sendXMLRequest("/general/crm/modules/table/intel_view/submit/dragconfig.php?MYTABLE="+escape(a),null)
}

function get_module_content(module_id, url)
{
   $("module_"+module_id+"_ul").innerHTML="<div style='text-align:center;'><img src='/static/images/loading.gif' align='absMiddle'> "+ td_lang.crm.inc.msg_60 +"</div>";
   var xmlHttpObj=getXMLHttpObj();
   ...
   xmlHttpObj.onreadystatechange=function()
	{
	 	if(xmlHttpObj.readyState==4)
	 	{
	 		if(xmlHttpObj.status==200)
	 		   ...
	 		   $("module_"+module_id+"_ul").innerHTML=td_lang.inc.msg_103+xmlHttpObj.status;
	 	}
	}
   xmlHttpObj.send(null)
}

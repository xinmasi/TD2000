...
function CreateXMLDom(XmlSrc) //创建XMLDOM对象函数，跨浏览器解决方案
{
   ...
   {
       window.alert(td_lang.crm.inc.msg_47);
       return null;
   }
...
function Tree(id, XmlSrc)
{
   ...
   this.Xml = {};
   this.icons = {
      'plus'         : '/static/images/collapsed.gif',
      'minus'        : '/static/images/expanded.gif',
      'transparent'  : '/static/images/transparent.gif',
      'loading'      : '/static/images/endnode.gif',
      'msg'          : '/static/images/msg.png',
      'email'        : '/static/images/email.png'
   };
   this.msg = {
      'loading'    : '<img align="absmiddle" src="' + this.icons['loading'] + '" class="NodeLoadingImg">'+td_lang.crm.inc.msg_48,
      'loadfailed' : '<img align="absmiddle" src="' + this.icons['loading'] + '" class="NodeLoadingImg">'+td_lang.crm.inc.msg_49,
      'parsefailed'    : td_lang.crm.inc.msg_50
   };
}
...
Tree.prototype.ClickNode = function(id)
{
   ...
   var expander = $(this.id + '_expand_' + id);
	...
	else if(curNode.className.indexOf('active') < 0 && (!container || container.style.display != 'block'))
...
Tree.prototype.Node2HTML = function(rootNode)
{
   ...
   for (var i=0;i<nodes.length;i++) //循环显示一级分类
   {
	  ...
	  html += '<span id="' + this.id + '_span_' + id + '" title="' + title + '" onmousedown="' + onmousedown + '" class="NodeSpan"';

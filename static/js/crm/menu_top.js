window.onload=function()
{
   ...
   for(var i=0; i<menu.childNodes.length;i++)
   {
      ...
      if(menu_id==0){
		 ...
		 sub_arr=(menu.childNodes[i].id).split("_");
		 var submenu=document.getElementById("submenu_"+sub_arr[1]);
		 if(submenu){
      ...
      menu.childNodes[i].onclick=function(){
         ...
         for(var i=0; i<menu.childNodes.length;i++)
         {
			if(menu.childNodes[i].className=="submenu"){

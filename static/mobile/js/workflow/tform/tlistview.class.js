...
//        },
    ...
    var ReportListFieldManager = TListViewManager.extend({
        ...
        parseVal: function(val,maxReminder){
            var multiplier =  window.Math.pow(10,maxReminder);
            val = val*multiplier;
            return val
        },
        devideVal: function(val,maxReminder) {
            var divide =  window.Math.pow(10,maxReminder);
            val = val/divide;
            return val;
        },
        calcSum: function(){//合计计算
            ...
            $.each(self.instances, function(k, v){
                $.each(v.cfg.item, function(k1, v1){
                    var tempRule = v1.code_select_value;
                    var tempVal = v1.colvalue;
                    if(self.validate_num(tempVal)){
                      if(tempRule.indexOf('|') != -1){
                          var tempReminder  = tempRule.substr(tempRule.indexOf('|')+1,1);//余数
                          maxReminder = tempReminder>maxReminder?tempReminder:maxReminder;
                      }else if(tempVal && tempVal.indexOf('.') != -1){
                          var tempArr = tempVal.split('.');
                          if(tempArr.length == 2 ){
                              var arrReminder = tempArr[1].length;
                              maxReminder = arrReminder>maxReminder?arrReminder:maxReminder;
                          }else{
            ...
            $.each(self.register.sumArr, function(i, obj){
                if(obj.sumflag == true){
                    	...
                    	if(v.cfg != undefined){
	                        ...
	                        $.each(v.cfg.item, function(k1, v1){
	                            if(v1.id == obj.id){
                                    var val = v1.colvalue;
	                                if(val == ""){val = 0;}
	                                if(self.validate_num(val)){
                                        sum += self.parseVal(parseFloat(val),maxReminder);
	                                }else{
	                                    isright = false;
	                                    return false;
	                                }
                    ...
                    if(flag == true){
                        sum = self.devideVal(parseFloat(sum),maxReminder);
                        if(rule.indexOf('|') != -1){
                            var _reminder  = rule.substr(rule.indexOf('|')+1,1);//余数
                            //obj.colvalue = sum.toFixed(_reminder);
...
';
        ...
        setValue: function(str, reminder,isMaxReminder){
            var res = math.eval(str);
			//console.log(res, reminder)
           // res = res/(window.Math.pow(10,isMaxReminder));
            if(reminder !== ""){
        ...
        calc: function(instances, rule, result, reminder){
            ...
            if(rule.indexOf('|')!=-1){
                reminder  = rule.substr(rule.indexOf('|')+1,1);//余数
                isMaxReminder = reminder>isMaxReminder?reminder:isMaxReminder;
                rule = rule.substr(0,rule.indexOf('|'));//提纯公式
            }
                ...
                $.each(arr, function(m, n){
                    if(v.cfg.id == ('list_'+list_id+'_col_'+n)){
                        ...
                        if(calv && calv.indexOf(".") != -1){
                            var arrCalv = calv.split('.');
                            if(arrCalv.length == 2 ){
                                var newarrReminder = arrCalv[1].length;
                                isMaxReminder = newarrReminder>isMaxReminder?newarrReminder:isMaxReminder;
                            }
                        }
                        ...
                        //calv = window.Math.pow(10,isMaxReminder)*calv;
                        rule = rule.replace(re,calv);
                    }
                });
            });
            instances[result].setValue(rule, reminder ,isMaxReminder);

    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data == "NOEDITPRIV") {
                ...
                return;
            } else if (data == "HAVEPADDING") {
                showMessage(havepadding);
                return;
...
function getflowContent(url, actionType) {
    $.ajax({
        ...
        url: url,
        cache: false,
        data: { 'RUN_ID': q_run_id, 'FLOW_ID': q_flow_id, 'PRCS_ID': q_prcs_id, 'FLOW_PRCS': q_flow_prcs, 'PRCS_KEY_ID': q_prcs_key_id, 'P': p, 'actionType': actionType, 'qrFlag': q_qr_flag, 'is_view':is_view, 'VIEW_CONTENT':VIEW_CONTENT },
        beforeSend: function() {
            $.ProLoading.show();
        },
        success: function(data) {
        ...
        success: function(data) {
			...
			$(".tworkflow").delegate(".setFormItemClass", "click", function() {
                ...
                $.ajax({
                    ...
                    url: 'set_form.php',
                    cache: false,
                    data: { 'RUN_ID': q_run_id, 'FLOW_ID': q_flow_id, 'PRCS_ID': q_prcs_id, 'FLOW_PRCS': q_flow_prcs, 'PRCS_KEY_ID': q_prcs_key_id, 'setItemId': setItemId, 'freeFormItemVal': freeItemVal },
                    beforeSend: function() {
                        $.ProLoading.show();
                    },
                    success: function(data) {
...
function goOnWorkFlow(new_prcs_id_next, turn_action) {
    ...
    var READ_USER_ID = $("input[name='READ_USER_ID']").val(); //传阅人
    ...
    if (prcs_id_next == 0 || typeof(prcs_id_next) == 'undefined') {
        ...
        if (TURN_PRIV == 1 && (NOT_ALL_FINISH != "" && NOT_ALL_FINISH != undefined && NOT_ALL_FINISH != null)) {
            if (confirm('经办人[' + NOT_ALL_FINISH + ']尚未办理完毕，确认要结束流程吗？')) {} else {
                return;
            }
        }
    }
    ...
    // });
    // }
    var hidden_data = $("div[name='HIDDEN_DATA']").html() == null ? '' : $("div[name='HIDDEN_DATA']").html(); //turn.php页面获取的一堆值需要传到turn_user.php页面
    var run_name = $("span.run_name").html();
    var begin_user = $("span.begin_user").html();
    $.ajax({
        type: 'POST',
        url: action,
        cache: false,
        data: { 'RUN_ID': q_run_id, 'FLOW_ID': q_flow_id, 'PRCS_ID': q_prcs_id, 'FLOW_PRCS': q_flow_prcs, 'PRCS_KEY_ID': q_prcs_key_id, 'PRCS_ID_NEXT': prcs_id_next, 'SMS_REMIND_NEXT': SMS_REMIND_NEXT, 'RX_PRCS': RX_PRCS, 'FLOW_TYPE': flow_type, 'FREE_STEP': freeStepVal, 'RUN_NAME': run_name, 'BEGIN_USER': begin_user, 'HIDDEN_DATA': hidden_data,'READ_USER_ID': READ_USER_ID },
        beforeSend: function() {
            $.ProLoading.show();
        },
        success: function(data) {
...
function turnUserWorkFlow(control_type) {
  ...
  //  var VIEW_USER_ID = $("input[name='VIEW_USER_ID']").val(); //传阅人
    ...
    if (RX_PRCS_LEN > 0 && $('input[prcs_type="'+PRCS_TYPE+'"]').prop('checked') === true) {
        rx_prcs = $("#rx_prcs").val();
    }
    if (prcs_back_len > 0) {
    ...
    var POST_STR = "RUN_ID=" + q_run_id + "&FLOW_ID=" + q_flow_id + "&PRCS_ID=" + q_prcs_id + "&FLOW_PRCS=" + q_flow_prcs + "&PRCS_KEY_ID=" + q_prcs_key_id + "&PRCS_ID_NEXT=" + prcs_id_next + "&FREE_STEP=" + free_step + "&READ_USER_ID=" + READ_USER_ID + "&PRCS_BACK=" + prcs_back + "&RX_PRCS=" + rx_prcs + "&DING=" + DING + plugin_str + "&NEXT_PRCS_ID=" + NEXT_PRCS_ID;
    if (free_step != "" && typeof(free_step) != undefined) {
...
function cancel_flow() {
    $.ajax({
        ...
        url: 'delete_work.php',
        cache: false,
        data: { 'RUN_ID': q_run_id, 'FLOW_ID': q_flow_id, 'FLOW_PRCS': q_flow_prcs, 'PRCS_ID': q_prcs_id },
        success: function(data) {
            if (data == "ALL") {

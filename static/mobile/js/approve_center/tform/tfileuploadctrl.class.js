define('TFileUploadCtrl', function(require, exports, module){
    ...
    var TFileUploadCtrl = Base.extend({
        ...
        initialize: function(config) {
            ...
            this.$ios_files = $("#IOS_UPLOAD_FILES_"+this.id+"");
        ...
        parseHtml: function(d) {
            ...
            var tplHTML = '<% if(secret){ %>' +
                ...
                '>上传图片</button>' +
                '<% if(platform == "client"){ %>' +
                '<button type="button" id="IOS_UPLOAD_FILES_<%=id%>" class="ui-btn IOS_UPLOAD_FILES" ' +
                '<% if(!writable){ %>' +
                'disabled' +
                '<% } %>' +
                '>上传附件</button>' +
        ...
        bindEvent: function() {
            ...
            this.$ios_files && this.$ios_files.on('click', function(){
                self.iosuploadFiles();
            });
        ...
        iosuploadFiles:function() {
            var self = this;
            tMobileSDK.selectFile({
                multiple: false,
                onSuccess: function(data) {
                    //alert(JSON.stringify(result))
                    self.$list.append('<a href="javascript:void(0);" data-attach_id="'+data[0].id+'" data-attach_name="'+data[0].name.replace("*","")+'" class="pda_attach pda_attach_file" _href="'+data[0].url+'"><span class="fileupload_attach_name">'+data[0].name.replace("*","")+'</span><span class="fileupload-item icon-delete">×</span></a>');
                    // loader.loading("hide");
                },
        ...
        getAttachInfo: function(imgServerId){
            ...
            $.ajax({
                type: 'GET',
                url: '/pda/approve_center/img_download.php',
                cache: false,
                data: {

...
function goOnWorkFlow(new_prcs_id_next, turn_action) {
    ...
    // });
    // }
    var hidden_data = $("div[name='HIDDEN_DATA']").html(); //turn.php页面获取的一堆值需要传到turn_user.php页面
    ...
    $.ajax({
        ...
        url: action,
        cache: false,
        data: { 'RUN_ID': q_run_id, 'FLOW_ID': q_flow_id, 'PRCS_ID': q_prcs_id, 'FLOW_PRCS': q_flow_prcs, 'PRCS_KEY_ID': q_prcs_key_id, 'PRCS_ID_NEXT': prcs_id_next, 'SMS_REMIND_NEXT': SMS_REMIND_NEXT, 'RX_PRCS': RX_PRCS, 'FLOW_TYPE': flow_type, 'FREE_STEP': freeStepVal, 'RUN_NAME': run_name, 'BEGIN_USER': begin_user, 'HIDDEN_DATA': hidden_data },
        beforeSend: function() {
            $.ProLoading.show();
        },
        success: function(data) {

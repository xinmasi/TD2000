				...
				var e = c.explorer(this.getCurState().json,{
					onDbclick: function(){
								...
								if(isFolder){								
									...
									G.openURL('process-copy-' + selectedId, '复制流程', '/general/system/workflow/flow_guide/flow_type/newflow.php?act=copy&entity_name=' + text + '&FLOW_ID=' + selectedId + '&SORT_ID=' + G.sortid);
								}
					}
				}).data('explorer');
				p.renderCtrlBar([
						{
							...
							func:function(){
								...
								if(isFolder){								
									...
									G.openURL('process-copy-' + selectedId, '复制流程', '/general/system/workflow/flow_guide/flow_type/newflow.php?act=copy&FLOW_ID=' + selectedId + '&SORT_ID=' + G.sortid);
								}
							}
						},
						{
	...
	*/
	...
	G.empty_form = function(FLOW_ID){
	    ...
	    if(window.confirm(msg)){
	        var url="/general/system/workflow/flow_guide/flow_type/empty.php?FLOW_ID="+FLOW_ID;
	...
	G.delete_form = function(FLOW_ID){
	    ...
	    if(window.confirm(msg)){
	        var url="/general/system/workflow/flow_guide/flow_type/delete.php?FLOW_ID="+FLOW_ID;
			ajax_update(url,{success: '删除成功！',fail: '删除失败！'},function(){
				window.location.href = "/general/system/workflow/flow_guide/index.php";	
			});
	    }
	}
	...
	G.moveData = function(){
		G.openPanel('/general/system/workflow/flow_guide/flow_type/data/?FLOW_ID=' + G.flowId ,{ }, { close:true });
	};
	// 侧边栏加载oa菜单
		...
		p.renderCtrlBar([
			{
				...
				func:function(){				
					...
					G.openURL('task-edit-'+selectedId, '编辑定时任务', '/general/system/workflow/flow_guide/flow_type/set_timer/new_timer.php?EDIT_FLAG=1&FLOW_ID=' + G.flowId + '&T_ID=' + selectedId);							
					p.close();
				}
			},{
				text:'删除',
				...
				func:function(){				
					...
					$.post('/general/system/workflow/flow_guide/flow_type/set_timer/del_schema.php', { TASK_ID: selectedId }, function(msg){
						if(msg == '+OK'){
							p.message('删除成功');							
							selectedNode.remove();
						} else {
		...
		p.renderCtrlBar([
			{
				...
				func:function(){				
					...
					G.openURL('print-edit-'+selectedId, '编辑版式文件', '/general/system/workflow/flow_guide/flow_type/set_print/new_tpl.php?FLOW_ID=' + G.flowId + '&SORT_ID=' + G.sortid + '&T_ID=' + selectedId);							
					p.close();
				}
			},{
				text:'删除',
				...
				func:function(){				
					...
					$.post('/general/system/workflow/flow_guide/flow_type/set_print/delete.php', { T_ID: selectedId }, function(msg){

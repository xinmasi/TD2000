            ...
            get_module_info: function(m){
                jQuery.ajax({
                    ...
                    success:function(d){
                        ...
                        if(k.indexOf('6') >= 0){
                            jQuery("#workflow_manager").empty();
                            jQuery("#workflow_manager").html(v["6"]);
                        }
                        if(p==1){
       ...
       if((window.external && typeof window.external.OA_SMS != 'undefined') || $("#login_theme").val() < 10)
		{
		   jQuery("[data-type='work_form']").click(function(){
				window.open("/general/system/workflow/flow_form");
			});
			
			jQuery("[data-type='work_flow']").click(function(){
				window.open("/general/system/workflow/flow_guide");
			});
			
			jQuery("[data-type='work_sort']").click(function(){
				window.open("/general/system/workflow/flow_sort");
			});
			jQuery("[data-type='work_hook']").click(function(){
				window.open("/general/system/workflow/flow_hook");
			});
		...
		else
		{
			jQuery("[data-type='work_form']").click(function(){
            	...
            	top.createTab(37,'设计表单','system/workflow/flow_form','');
			});
			jQuery("[data-type='work_flow']").click(function(){
				top.createTab(135,'设计流程','system/workflow/flow_guide','');
			});
			jQuery("[data-type='work_sort']").click(function(){
				top.createTab(136,'分类设置','system/workflow/flow_sort','');
			});
			jQuery("[data-type='work_hook']").click(function(){
				top.createTab(253,'业务引擎设置','system/workflow/flow_hook','');
			});

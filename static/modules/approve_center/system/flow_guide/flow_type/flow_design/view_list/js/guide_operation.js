...
function prcs_type()
{
    ...
    else if(typeVal=='3')
    {
		...
		//jQuery('#time_out_auto_turn').hide(); //超时自动转交隐藏
    ...
    else 
    {
		...
		jQuery('#user_name_ext').hide();
		jQuery('#user_name_in').hide();
        
    }   
}
//通过是否强制并发，来控制强制合并选项的显隐
function gather_set() {
    var merge_val = jQuery('input[name="GATHER_NODE"]:checked').val();
    if(merge_val == 1)
    {
...
function sub_form_ext(flow_id)
{
	var prcs_type_flag = jQuery("#PRCS_TYPE").val();
	var flowListExt = jQuery('#flow_list_ext').find('option:selected').val();
	// var flow_list_ext_hidden = jQuery("#flow_list_ext_hidden").val();
	var id = jQuery('#EXT_ID_VAL').find('option:selected').val();
	if(prcs_type_flag == 4)
	{
		if(flow_id >= 0)
		{
			jQuery("#field_sub_ext").load("get_field_ext1.php?FLOW_ID="+flow_id+"&server_id="+id);
			jQuery('.mappings_ext').show();
		}
	}else
	{
		jQuery('.mappings_ext').hide();
	}
	//如果更换流程、删除已存在的映射关系
	if(flow_list_ext != flow_id)
	{
		jQuery("#RELATION_IN_EXT").empty();
		jQuery("#MAP_IN_EXT").val("");
		jQuery("#MAP_IN_UID_EXT").val("");
		jQuery("#flow_list_ext_hidden").val(flowListExt);
	}
}
//添加内外流程映射
function map_relation_ext()
{
	// var map_uid = jQuery('#FLD_PARENT_EXT').find('option:selected').val();
	var map_name = jQuery('#FLD_PARENT_EXT').find('option:selected').text();
	// var map_uid_ext = jQuery('#FLD_SUB_EXT').find('option:selected').val();
	var map_name_ext = jQuery('#FLD_SUB_EXT').find('option:selected').text();
	var exists_data = jQuery('#RELATION_IN_EXT').text();
	var map_relation_data = '';
	if(map_name == '' || map_name_ext == '')
	{
		alert("请正确选择映射关系");
	}else
	{
		map_relation_data = map_name+'=>'+map_name_ext+',';
		if(exists_data != '' && exists_data.indexOf(map_relation_data) > -1)
		{
			alert("已经添加此映射,请重新添加");
		}else
		{
			map_add(map_relation_data,"IN_EXT");
		}
	}
}
...
function file_flow(direction)
{
    var option1,option2;
    var oFLD_PARENT = document.getElementById("FLD_FILE");
    var oFLD_SUB = document.getElementById("FLD_FLOW");
    if(oFLD_PARENT.selectedIndex!=-1&&oFLD_SUB.selectedIndex!=-1)
    {
        option1=oFLD_PARENT.options[oFLD_PARENT.selectedIndex].value;
        option2=oFLD_SUB.options[oFLD_SUB.selectedIndex].value;
...
function file_add(data,direction){
	var addhtml;
	...
	for(i=0;i<rel_array.length-1;i++) {
		addhtml="<span>"+rel_array[i]+"<a href=\"javascript:\" onclick=\"file_del(this,'"+i+"','"+direction+"')\">"+
...
function file_del(a,index,direction)
{
...
function myload()
{
	...
	{
		sel_back();
	}else if(prcs_type_flag == 4)
	{
		sub_form_ext(flow_list_ext);
		jQuery("#flow_name").val(flow_list_ext_name);
	}	
	if(secu == 1 && workflow_setuser != 1 && workflow_normal == 1){
		auto_set();
		filter_auto_set();
...
{
	var prcsType = jQuery("#PRCS_TYPE").val();
	if(prcsType == 1)
	{
		var span = a.parentNode;
		span.parentNode.removeChild(span)
	...
	}else
	{
		...
		var hiddenText = jQuery('#RELATION_IN_EXT').text();
		jQuery("#MAP_"+direction).val(hiddenText);
	}
}
...
function check_write()
{
    ...
    jQuery('input[list_column_id][mode_type=readOnly]').each(function(index,element){
        ...
        var required_hidden_obj = jQuery('input[list_column_id='+list_column_id+'][mode_type=requiredOnly]');
        var readOnly = 0;
        var hidden = 0;
		var requiredOnly = 0;
        if(element.checked)
		...
		if(required_hidden_obj.length != 0 && required_hidden_obj[0].checked)
        {
            requiredOnly = required_hidden_obj[0].value;
        }
        jQuery('#'+list_column_id).val(readOnly | hidden | requiredOnly);
    });
	return true;
}
//新建、保存步骤信息
...
{
	var flow_list_ext_hidden = jQuery("#flow_list_ext_hidden").val();//外部流程
	// jQuery('#flow_list_ext_hidden').val(flowIdExt);
	...
	//外部流程验证
	if(prcs_type == 4 && flow_list_ext_hidden == '')
	{
		alert("请选择外部操作流程");
		return false;
	}
...
function test_link()
{
	...
	jQuery('#textRedition').hide();
	jQuery('#textReditionHeader').hide();
	var id = jQuery('#EXT_ID_VAL').find('option:selected').val();
	var data = "ServerId="+id;
	var flow_list_ext = jQuery('#flow_list_ext_hidden').val();
	var url = 'test_link.php';
	var tryCount = 0;
	var interval = setInterval(function(){
			tryCount++;
			if(tryCount==2)
			{
				alert('请求数据失败,请检查参数配置');
			...
			beforeSend: function ()
			{
				jQuery('.next_step').attr('disabled',true);
			},
			success:function(flag)
			{
				...
				{
					clearInterval(interval);
					jQuery('#flow_list_ext').empty();
					jQuery('#nocbox_tips').html("");
					jQuery('#textRedition').show();
					jQuery('#textReditionHeader').show();
					jQuery.getJSON('/general/approve_center/get_flow_list_ext.inc.php',{FLOW_ID_EXT:flow_list_ext,server_id:id},function(jsonData){
						jQuery("#flow_list_ext").append('<option value=""></option>');
						jQuery.each(jsonData,function(i,t){
							jQuery("#flow_list_ext").append('<option value="'+t.value+'" category="'+t.category+'" node="'+t.node+'" '+t.selected+' level="'+t.level+'">'+t.txt+'</option>');
						});
						jQuery("#flow_list_ext").combobox();//此页面表示将SELECT控件进行标准处理
						var href_id = jQuery('.active').parent().find("li[class$='active']").find("a").attr("href");
						if(typeof(href_id) != "#list")
						{
							myload();
			...
			prcs_guide +='<ul class="nav nav-list bs-docs-sidenav" id="address">';
				prcs_guide +='<li class="active" item="first" index="1" id="basis_li">';
					prcs_guide +='<a href="#basis" data-toggle="tab">';
						...
						prcs_guide +='<span class="icon20-basis_hover">'+td_lang.system.workflow.msg_basis+'</span></a>';
				prcs_guide +='</li>';
				prcs_guide +='<li class="" item="last" index="2" id="select_li">';
					prcs_guide +='<a href="#select" data-toggle="tab">';
						prcs_guide +='<i class="icon-chevron-right" style="margin-top:8px;"></i>';
						prcs_guide +='<span class="icon20-select">'+td_lang.system.workflow.msg_select+'</span></a>';
				prcs_guide +='</li>';
			prcs_guide +='</ul>';
		}
		else

jQuery(document).ready(function()
{
	...
	var select_value = window.select_value = jQuery('#select_value_hidden').val();
	form_sort_parent(sort_id,sort_name,'',select_value,'',window.form_id);
...
//工作流表单升级
function form_workflow_up()
{
	var msgStr = "升级表单前，先将数据库备份。是否继续";
...
{
    var select_value = window.select_value && window.selct_value != '' ? window.select_value : '';
	...
	if(TYPE == 1)
	{
		document.getElementById("form_news").action = "/general/system/approve_center/flow_form/insert.php";
	}
	else
	{
		document.getElementById("form_news").action = "/general/system/approve_center/flow_form/update.php?type="+TYPE+"&select_value="+select_value;
	}
...
//表单设计器
function form_smart(form_id,flow_id,flow_prcs)
{
    if(window.flow_id && window.flow_id != 0)
    {
        window.open("cool_form/ue.php?FORM_ID="+form_id+"&FLOW_ID="+window.flow_id,"cool_form","menubar=0,toolbar=0,scrollbars=no,status=0,resizable=1");
    }
    else
    {
...
function form_preview(form_id)
{
	...
	window.open("/general/approve_center/form_view.php?FORM_ID="+form_id,"form_view_"+form_id,"menubar=0,toolbar=0,status=0,resizable=1,left=0,top=0,scrollbars=1,width="+(screen.availWidth-10)+",height="+(screen.availHeight-50)+"\"");
...
//导入
function form_import(form_id, flow_id, flow_prcs)
{
	jQuery('#form_edit_div').html("");
	jQuery('#form_new_div').html("");
	...
	if(window.flow_id && window.flow_id != 0)
	{
        jQuery('#form_import_div').load('import.php?form_id='+form_id+'&flow_id='+window.flow_id,function()
        {
...
{
	open_bootcss_modal("myModal_version","700","4");
    if(window.flow_id && window.flow_id != 0)
    {
        jQuery('#form_version_div').load('version.php?FORM_ID='+form_id+'&FLOW_ID='+window.flow_id,function()
        {
            version_render();
        });
    }
    ...
    {
        jQuery('#form_version_div').load('version.php?FORM_ID='+form_id,function()
        {
            version_render();
        });
    }
...
{
	// window.location="/general/system/approve_center/flow_form/vsn_form_view.php?FORM_ID="+form_id+"&ID="+vid;
	//window.open("/general/system/approve_center/flow_form/vsn_form_view.php?FORM_ID="+form_id+"&ID="+vid,	"menubar=0,toolbar=0,status=0,resizable=1,left=0,top=0,scrollbars=1,width="+(screen.availWidth-10)+",height="+(screen.availHeight-50)+"\"");
	window.open("/general/approve_center/form_view.php?FORM_VERSION_ID="+vid,	"menubar=0,toolbar=0,status=0,resizable=1,left=0,top=0,scrollbars=1,width="+(screen.availWidth-10)+",height="+(screen.availHeight-50)+"\"");
}
//恢复版本
function set_main_vsn(vid)
{
	...
	if(window.confirm(msg))
	{
        ...
        {
            jQuery.ajax({
                url: "vsn_set_main.php?FORM_ID="+form_id+"&FLOW_ID="+window.flow_id+"&ID="+vid,
                data: "ID="+vid,
                type: "POST",
                async: true,
                success: function(data){
	...
	if(window.confirm(msg))
	{
		...
		({		
			...
			success: function(data){
				...
				}else if(data == "workUsed"){
                    alert('此表单版本已建立工作，不能删除！');
				}					
			},
			error: function(data){
				alert(data.responseText);
...
//编辑版本
function form_version_edit(vid)
{
    if(window.flow_id && window.flow_id != 0)
    {
        window.open("cool_form/ue.php?VERSION_FORM_ID="+vid+"&FLOW_ID="+window.flow_id,"cool_form","menubar=0,toolbar=0,scrollbars=no,status=0,resizable=1");
    }
    else
    {
        window.open("cool_form/ue.php?VERSION_FORM_ID="+vid,"cool_form","menubar=0,toolbar=0,scrollbars=no,status=0,resizable=1");
    }
}

//表单版本移动智能设计器
function form_version_smart(vid)
{
    window.open("pda/index.php?version_form_id="+vid);
}
//表单版本导出
function form_version_export(vid)
{
    var msg1=td_lang.general.workflow.msg_170;
    if(window.confirm(msg1))
    {
        window.location.href="export.php?VERSION_FORM_ID="+vid;
    }
}

function use_version(vid)
{
    msg= "启用后表单将以此表单版本为准，确定启用此版本么？";
    if(window.confirm(msg))
    {
        jQuery.ajax
        ({
            url: "version_use.php?FORM_ID="+form_id+"&ID="+vid,
            data: "ID="+vid,
            type: "POST",
            async: true,
            success: function(data){
 ...
 */
function form_sort_parent(sort_id,sort_name,compositor_id,select_value,search_area,form_id)
{
	...
	({		
		...
		data: "sort_id="+sort_id+"&form_id="+form_id,
		type: "POST",
		async: true,
		dataType: 'json',
		success: function(data){
...
//							}
							jQuery('#form_list1').find('table').append('<tr form_id='+jsonData[one][key].form_id+'><td><span class="icon16-pencil-content">'+jsonData[one][key].form_name+'</span></td><td><a class="form_ahref" href="javascript:form_edit('+jsonData[one][key].form_id+')">'+td_lang.general.workflow.msg_64+'</a><a class="form_ahref" href="javascript:form_smart('+jsonData[one][key].form_id+','+flow_id+','+flow_prcs+')">'+td_lang.general.workflow.msg_65+'</a><a class="form_ahref" href="javascript:form_mobile_smart('+jsonData[one][key].form_id+')">'+td_lang.general.workflow.msg_mobile+'</a><a class="form_ahref" href="javascript:form_preview('+jsonData[one][key].form_id+')">'+td_lang.general.workflow.msg_66+'</a><a class="form_ahref" href="javascript:form_import('+jsonData[one][key].form_id+', '+flow_id+')">'+td_lang.general.workflow.msg_67+'</a><a class="form_ahref" href="javascript:form_export('+jsonData[one][key].form_id+')">'+td_lang.general.workflow.msg_68+'</a><a class="form_ahref" href="javascript:form_version('+jsonData[one][key].form_id+')">'+td_lang.general.workflow.msg_69+'</a>'+del_flag+'</td></tr>');
							J++;
						}
					}
					else //图形视图
...
function changeImage(cookieIcon)
{
	...
	var sort_name = jQuery('#sort_name_hidden').val();
	var select_value = jQuery('#select_value_hidden').val();
	form_sort_parent(sort_id,'','',select_value,'',window.form_id);
}

...
function CheckForm(callback)
{
	...
	setTimeout(function(){
		...
		{
			var windowflowname = window.flow_name;
			var variable = new Array();
			jQuery.ajax({
				type: 'POST',
				async: false,
				url: './controls/inc/variable.php',
				data: {
					FLOW_ID: window.flow_id || 0,
					FORM_ID:window.form_id || 0
				},
				success: function(data){
					variable = data;
			...
			//从缓存里预加载uid(html里没有uid属性时会用到预加载uid)
            /*var cacheUids = new Array();
            jQuery.ajax({
                type: 'POST',
                async: false,
                url: './controls/inc/variable_cache.php',
                data: {
                    FORM_ID: window.form_id || 0
                },
                success: function(data){
			...
			for(var i=0;i<size;i++){
				with(docElements[i]){
					...
					var _uid = getAttribute("uid") || getAttribute("variable_sort_uid");
					var listtitle = getAttribute("lv_title") || '';
					var listuid = getAttribute("variable_id") || '';
					...
					if(jQuery.inArray(_tagName,tagArr) == -1){
						...
						if(filedTag == 'td_calendar' || filedTag == 'td_websign' || filedTag == 'td_mobilesign' || filedTag == 'td_mobilewritesign' || filedTag == 'td_qrcode' || filedTag == 'td_orgselect')
						{
							if((filedTag == 'td_calendar' || filedTag == 'td_orgselect'))
							{
								_title = _title;
							}else
							{
						...
						//针对所有控件进行变量筛选
						nameflag = windowflowname;
						nameflag += ".";
						nameflag += _title;
						var titleflag = variable[nameflag];
						if(filedTag == 'td_listview')
							titleflag = variable[_title];
						if(titleflag != '' && typeof(titleflag) != 'undefined')
						{
							var _uidsave = titleflag;
							if(_uidsave != _uid)
							{
								var msg = td_lang.system.workflow.msg_58;
								msg += sprintf(td_lang.system.workflow.msg_59, nameflag);	
								alert(msg);
								ret = false;
								continue;
							}
						...
						// {
							...
							if(jQuery.inArray(_uid,uidArr) > -1)
							{
								var msg = td_lang.system.workflow.msg_58;
								msg += sprintf(td_lang.system.workflow.msg_59, nameflag);	
								alert(msg);
						...
						if(filedTag == 'td_listview')
						{
							var titlename = _title;
							if(listuid != '')
							{
								jQuery.each(variable,function(i,v){
									if(v == _uid)
									{
										titlename = i;
									}
								});
								listuid = listuid.substring(0,listuid.length-1);
								listtitle = listtitle.substring(0,listtitle.length-1);
								var listuidarr = listuid.split("`");
								var listtitlearr = listtitle.split("`");
								var rowarr = new Array();
								for(var j=0;j<listuidarr.length;j++)
								{
									var msg = td_lang.system.workflow.msg_58;
									if(jQuery.inArray(listtitlearr[j],rowarr) > -1 || jQuery.inArray(listuidarr[j],uidArr) > -1)
									{
										msg += sprintf(td_lang.system.workflow.msg_59,titlename+'.'+listtitlearr[j]);	
										alert(msg);
										ret = false;
										continue;
									}
									uidArr.push(listuidarr[j]);
									rowarr.push(listtitlearr[j]);
									var listrowname = titlename + "." + listtitlearr[j];
									if(variable[listrowname] != '' && typeof(variable[listrowname]) != 'undefined')
									{
										var listrowvalue = variable[listrowname];
										if(listrowvalue != listuidarr[j])
										{
											msg += sprintf(td_lang.system.workflow.msg_59, listrowname);	
											alert(msg);
											ret = false;
											continue;
										}
...
function send(TYPE)
{
	CheckForm(function(flag){
		if(flag)
		{
			...
			{
               // var msg='<?=_("关闭表单设计器前，保存对表单的修改？")?>';
              //  if(window.confirm(msg))
               // {
                    document.form1.action = "submit.php?TYPE=2&e=ue";
             //   }
              //  else
            //    {
              //      window.close();

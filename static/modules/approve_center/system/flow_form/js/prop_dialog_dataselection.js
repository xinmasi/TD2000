...
function _initCtrlProp(el)
{
    ...
    var uid = jQuery(el).attr('uid');
    //html中没有UID属性，这里在缓存中取出UID(工作流表单升级到流程中心时,或工作流表单导入到流程中心时)
    if(uid === '' || uid === null || uid === 0 || uid === false || uid === undefined)
    {
    ...
    jQuery('#txtVariable').find('option[value="'+uid+'"]').prop('selected', true);
